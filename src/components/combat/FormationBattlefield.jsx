...
import "./archetype-combat-formation.css";
...
function UnitVitals({ actor, enemy, presented }) {
  ...
  return (
    <span className="combat-formation-unit__vitals">
      <span
        className={`combat-formation-unit__meter combat-formation-unit__meter--hp${enemy ? " is-enemy" : ""}`}
        role="meter"
      ...
      >
        ...
        <span className="combat-formation-unit__meter-label" aria-hidden="true">HP</span>
        <span className="combat-formation-unit__meter-value" aria-hidden="true">
          <strong>{vitalText(hp)}</strong>/{vitalText(maxHp)}
        </span>
      </span>
      {hasResolve ? (
        <span
          className="combat-formation-unit__meter combat-formation-unit__meter--resolve"
          role="meter"
        ...
        >
          ...
          <span className="combat-formation-unit__meter-label" aria-hidden="true">RP</span>
          <span className="combat-formation-unit__meter-value" aria-hidden="true">
...
function FormationUnit({
  ...
  const presentationStyle = {
    ...(movement ? {
      "--combat-move-offset": moveOffset,
      "--combat-move-duration": `${movement.durationMs}ms`,
    } : {}),
    ...(lunge ? {
      "--combat-lunge-x": `${lunge.dx}px`,
      "--combat-lunge-y": `${lunge.dy}px`,
      "--combat-lunge-mid-x": `${lunge.dx * 0.74}px`,
      "--combat-lunge-mid-y": `${(lunge.dy * 0.74) - leap}px`,
      "--combat-lunge-wind-x": `${lunge.dx * -0.04}px`,
      "--combat-lunge-wind-y": `${lunge.dy * -0.04}px`,
      "--combat-lunge-delay": `${lunge.delayMs}ms`,
      "--combat-lunge-duration": `${lunge.durationMs}ms`,
    } : {}),
  };
  return (
    <span
      className={`combat-formation-unit${active ? " is-active" : ""}${down ? " is-down" : ""}${reacting ? " is-reacting" : ""}${movement ? " is-arriving" : ""}${lunge ? " is-lunging" : ""}`}
      style={Object.keys(presentationStyle).length > 0 ? presentationStyle : undefined}
      data-actor-id={actor.id}
      data-lunge-id={lunge?.id || undefined}
    >
      {overlay ? (
        <span className="combat-formation-cell__overlays">
          {overlay}
        </span>
      ) : null}
      {active && !down ? (
        <span className="combat-formation-unit__active-label" aria-hidden="true">
          <strong>Acting</strong>
          <em>{actor.name}</em>
        </span>
      ) : null}
      {down ? (
        <span className="combat-formation-unit__down-label" aria-hidden="true">Defeated</span>
      ) : null}
      <span className="combat-formation-unit__figure" aria-hidden="true">
        {art ? (
          <img src={art} alt="" draggable="false" decoding="async" />
        ) : (
          <span className="combat-formation-unit__monogram">
...
function FormationGrid({
  ...
  return (
    <section
      className={`combat-formation-grid combat-formation-grid--${side}${sideIsLunging ? " has-lunging-unit" : ""}`}
      aria-label={title}
    >
      {formation.map((entry, index) => {
        ...
        return (
          <button
            key={key}
            type="button"
            className={`combat-formation-cell ${stateClasses}`}
            data-side={side}
            ...
            onBlur={(event) => {
              ...
              if (next?.matches?.(".combat-formation-cell.is-valid-anchor:not(:disabled)")) {
            ...
            onMouseLeave={(event) => {
              ...
              if (focused?.matches?.(".combat-formation-cell.is-valid-anchor:not(:disabled)")) {
            ...
            style={destinationMove ? {
              "--combat-move-duration": `${destinationMove.durationMs}ms`,
            } : undefined}
          >
            {destinationMove ? (
              <span
                className="combat-formation-cell__move-marker"
...
export function FormationBattlefield({
  ...
  return (
    <section
      ref={battlefieldRef}
      className={`combat-formation-battlefield ${className}`.trim()}
    ...
    >
      <span
        className="combat-formation-battlefield__announcement"
        aria-live="polite"
        aria-atomic="true"
      >
        {announcement}
      ...
      />
      <span className="combat-formation-battlefield__divide" aria-hidden="true" />
      <FormationGrid
        side="player"
        actors={actors}
