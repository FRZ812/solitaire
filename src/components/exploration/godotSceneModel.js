...
export function buildWorldGodotScene({ model, selection, journey, night = false }) {
  return {
    version: 1,
    mode: "world",
    ...
    cells: model.viewport.map((cell) => ({
      ...
      col: cell.col,
      row: cell.row,
      // Unknown geography is deliberately normalized before it crosses the
      // renderer boundary. Fog can never leak a road, river, or settlement.
...
export function buildCityGodotScene({ model, current, selected, districtColors, night = false }) {
