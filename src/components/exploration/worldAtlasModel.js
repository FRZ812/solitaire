...
// ---- Tabletop tilt ----
//
// The 3D atlas leans the whole map plane back like a campaign table. The DOM
// applies `perspective(perspective)` on the scene and `rotateX(angleDeg)` on
// the plane, both about the stage center; the helpers below are the exact
// inverse of that projection so pointer picking, panning, and pinch zoom agree
// with the rendered pixels. Screen points are stage-relative; plane points are
// plane-relative, with the plane centered on the stage.

export const ATLAS_TILT = Object.freeze({
  angleDeg: 26,
  perspective: 1250,
});

function tiltCenterScreenToPlane(sx, sy, tilt) {
  const angle = (tilt.angleDeg * Math.PI) / 180;
  const sin = Math.sin(angle);
  const cos = Math.cos(angle);
  const v = (sy * tilt.perspective) / (tilt.perspective * cos + sy * sin);
  const u = (sx * (tilt.perspective - v * sin)) / tilt.perspective;
  return { x: u, y: v };
}

export function tiltPlaneToScreen(point, viewport, planeViewport, tilt = ATLAS_TILT) {
  if (!tilt) return { x: point.x, y: point.y };
  const angle = (tilt.angleDeg * Math.PI) / 180;
  const u = point.x - planeViewport.width / 2;
  const v = point.y - planeViewport.height / 2;
  const scale = tilt.perspective / (tilt.perspective - v * Math.sin(angle));
  return {
    x: u * scale + viewport.width / 2,
    y: v * Math.cos(angle) * scale + viewport.height / 2,
  };
}

export function tiltScreenToPlane(point, viewport, planeViewport, tilt = ATLAS_TILT) {
  if (!tilt) return { x: point.x, y: point.y };
  const centered = tiltCenterScreenToPlane(
    point.x - viewport.width / 2,
    point.y - viewport.height / 2,
    tilt,
  );
  return {
    x: centered.x + planeViewport.width / 2,
    y: centered.y + planeViewport.height / 2,
  };
}

// The tilted plane recedes at the top, so a plane the size of the stage would
// leave bare corners once rotated. This returns the centered, oversized plane
// that still covers every stage pixel after the rotation.
export function atlasPlaneViewport(viewport, tilt = ATLAS_TILT) {
  if (!tilt) return { width: viewport.width, height: viewport.height };
  ...
  for (const [sx, sy] of [
    ...
    const corner = tiltCenterScreenToPlane(sx, sy, tilt);
    maxU = Math.max(maxU, Math.abs(corner.x));
    maxV = Math.max(maxV, Math.abs(corner.y));
  }
  return { width: Math.ceil(maxU * 2), height: Math.ceil(maxV * 2) };
}
