...
import { buildCityMapScene, buildWorldMapScene } from "./mapSceneModel.js";
...
describe("browser exploration scene contract", () => {
  ...
  it("uses the same city cells for rendering, locations, and routes", () => {
    ...
    const scene = buildCityMapScene({ model, current: model.viewport[0].node, selected: model.viewport[2].node, districtColors: { Ward: "#abcdef" } });
