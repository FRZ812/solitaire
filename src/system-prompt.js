...
- [INSPECT CAPTIVE]: the player has moved along the line at the Block to look closer at one of the Chain Factor's lots. This OPENS the inspection and haggle — do NOT settle the sale on the approach. Follow THE BLOCK doctrine above: voice the Chain Factor reading the slate; voice the captive sparingly (they do not speak unless addressed); the listed bond is the OPENING price and the Factor will haggle within reason (engine floor 50% of list ON COIN ONLY). Across multiple turns the player chats, the Factor counters, the captive's freedom_response and refusal-doctrine remain in force for any actual freedom offer. ONLY when the settlement is settled — coin agreed, OR a non-coin settlement the Factor accepts in fiction (writ / ruse / theft / gift / barter, per THE BLOCK doctrine above) — and the Factor strikes the irons (or the captive is otherwise yours) set purchase_captive:{"key":"<captive.key>","agreedPriceCp":<final copper>,"settlement":"coin|writ|ruse|theft|gift|barter (default coin)","settlementNote":"<one-line act-description, required for non-coin>"}; the engine takes coin only when settlement is coin, files the bonded codex entry with the settlement recorded, and adds the captive to the party. If the player walks away, narrate the close and emit nothing.
