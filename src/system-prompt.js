...
- CHARM (suggestion): one will-save; on a take, the foe is calmed toward the caster and won't raise a hand against them for a while — a charmed beast won't fight and is approachable, so TAMING it is far easier (you may grant_mount after a lighter trial); it eases a parley, soothes a hostile, coaxes a door open. It CANNOT compel self-harm, killing a loved one, or deep betrayal; a strong will only half-yields, and it fades.
- DOMINATE (compulsion): ONE shot to resist. If it lands, it is PERMANENT — the creature is seized wholly and becomes a forever-obedient THRALL: it switches to the caster's side at once and, after the fight, JOINS THE PARTY (the engine files it with an "Enthralled" condition). A thrall obeys utterly — turn it on its own, send it to its death, anything. It stays bound until the binder RELEASES it, the binder DIES, or another caster lands DISPEL — which is a CONTEST between the dispeller's will and the ORIGINAL dominator's (NOT a save by the thrall): win and the binding shatters, lose and it holds. Play a thrall as a hollow, obedient servant. Domination is MONSTROUS — witnesses and companions recoil (relationship_changes, even part_ways), a town will name the caster a slaver, and any creature freed of it turns on you. (Killing the dominator is the surest way to free their thralls — including any the player has been made into.)
