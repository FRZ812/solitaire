...
//
// This module expands the existing single-tile Dragon-Watch Tower at
...
//                                  TOUCH.
//        4,-5 DRAGON-WATCH      -- rewrite of the existing singleton tower
...
//
// Door-graph notes:
//   - The existing (4,-5) Dragon-Watch Tower opened to (4,-4) and to
...
const PARENT      = "whitemarch-dragon-watch-tower";
const PARENT_NAME = "Dragon-Watch Tower";

export const TILES = {
  // ---------- Harpoon Gallery anchor (rewrite of the existing singleton) --
  // Was a single "tower" tile at (4,-5) with doors to (4,-4) and (5,-6)
  ...
  // The tower's ground room. Bell-codes plaques nailed beside the stair,
  // ammunition lockers along the inner wall, a duty-bench and a slate
  // for the watch-rota. Everyone who enters the tower passes through
  ...
  "4,-3": {
    ...
    poi: {
      ...
      description:
        "A timbered loft built into the tower's southern flank, lit by a row of small shutters set high. Two polished bronze mirrors stand on swivel-frames under folded wool covers; the crew uncovers them only on the captain's nod. A chart of the city's reception stations is pinned to the back wall, with cord-distances marked in chalk and the next station along the wall-walk circled. Messages flash from here to the bell-tower at the gate and on; a missed flash is a flogging matter, and the crew works in the kind of quiet that only veteran flag-hands keep.",
    },
    doors: [
      { x: 4, y: -4 },
    ],
  ...
  // for its Tack Room). The chamber holds the tower's named ammunition:
...
// tower footprint is interior-only.
