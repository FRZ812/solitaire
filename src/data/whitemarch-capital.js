...
export const WHITEMARCH_MAP_VERSION = 2;
...
// Fifty-four civic, commercial, religious, military, and neighbourhood POIs.
// The six gates below bring the named total to sixty. Exactly twenty-two of
// these point at BUILDINGS entries whose UI kinds are currently supported.
const POI_SPECS = Object.freeze([
  // Grand Market (5; 3 services)
  poi("grain-square", "Grain Square", "grand-market", "The broad central square where grain bells, public notices, and market cries order the capital's day.", { type: "market", service: "market", coord: { x: 0, y: 0 } }),
  ...
  poi("counting-house", "The Counting House", "grand-market", "A severe hall of scales and iron-bound ledgers where the crown's shillings are weighed and recorded."),
  poi("market-annex", "Provisioners' Arcade", "grand-market", "A roofed arcade of food stalls and expedition suppliers serving travellers before the road.", { type: "market", service: "market" }),
  poi("money-changer", "The Brass Scales", "grand-market", "A guarded exchange counter where foreign coin is clipped, tested, and traded by posted weight.", { type: "market", service: "money-changer" }),

  // Temple Steps (5; 2 services)
  poi("hospital-cloister", "Hospital Cloister", "temple-steps", "A clean stone cloister where lay healers dress wounds beneath rows of drying herbs.", { type: "healer", service: "healer" }),
  poi("oath-temple", "Temple of the Seven Oaths", "temple-steps", "A many-doored temple where civic, household, military, and funerary vows are witnessed."),
  poi("bonepicker-chapel", "Bonepicker Chapel", "temple-steps", "A soot-dark chapel whose keepers name the unclaimed dead before the city buries them."),
  ...
  poi("apothecary-stall", "Apothecary's Window", "temple-steps", "A green-shuttered dispensary selling measured powders, tinctures, poultices, and field remedies.", { type: "healer", service: "apothecary" }),

  // Low Wards (5; 2 services)
  ...
  poi("ward-store", "Low Ward General Store", "low-wards", "A crowded counter of lamp oil, rope, blankets, cheap tools, and household necessities.", { type: "market", service: "general-store" }),

  // Chain Ward (5; 2 services)
  ...
  poi("prison-gate", "Prison Gate", "chain-ward", "A nail-scarred threshold watched day and night by gaolers with ringed keys."),
  poi("bond-registry", "Bond Registry", "chain-ward", "A ledger hall where contracts of debt, service, ransom, and manumission are copied."),
  poi("auction-court", "Auction Court", "chain-ward", "A walled court of tally rails, inspection awnings, and hard-eyed licensed factors."),

  // Guild Court (5; 1 service)
  poi("guild-court", "Guild Court", "guild-court", "A formal square fronted by the chartered trades and their painted company signs."),
  poi("registry-hall", "Registry Hall", "guild-court", "A dry labyrinth of clerks, petitions, stamped copies, and chained reference books."),
  ...
  poi("charter-archive", "Charter Archive", "guild-court", "A guarded archive preserving guild privileges, civic boundaries, and judgments on trade."),
  poi("foreign-exchange", "Foreign Factors' Hall", "guild-court", "A multilingual trading room where licensed factors broker distant wares and letters of credit.", { type: "market", service: "foreign-trader" }),

  // River Docks (5; 3 services)
  poi("river-stair", "Whitewend Ferry Stair", "river-docks", "Broad wet steps descending to ferries, workboats, and the brown current of the Whitewend.", { coord: { x: 3, y: -4 } }),
  poi("high-quay", "High Quay Fish Market", "river-docks", "A wind-cut quay where fishmongers sell the morning catch under striped canvas.", { type: "market", service: "fishmonger" }),
  poi("warehouse-row", "Warehouse Row", "river-docks", "A canyon of hoists, bonded storehouses, tally doors, and shouting cargo crews."),
  poi("chandlery", "The Lamp and Line Chandlery", "river-docks", "A tar-scented shop of cordage, canvas, lamp fuel, hooks, floats, and shipboard tools.", { type: "market", service: "chandler" }),
  poi("cooperage", "Whitewend Cooperage", "river-docks", "A hammering yard where barrels, tubs, and casks are raised from seasoned staves.", { type: "market", service: "cooper" }),

  // Iron Quarter (5; 4 services)
  poi("smith-row", "Public Smith Row", "iron-quarter", "A long forge lane ringing with hammers and lit orange by banked working fires.", { type: "smithy", service: "blacksmith" }),
  poi("forge-yard", "Crown Forge Yard", "iron-quarter", "A soot-black work court for military repairs, wagon iron, and bulk contracts."),
  poi("leather-row", "Leather Row", "iron-quarter", "Tanners and harness makers work beneath awnings sharp with bark, oil, and curing hide.", { type: "market", service: "leather-worker" }),
  poi("cartwright-yard", "Cartwright Yard", "iron-quarter", "A timber yard of hubs, spokes, axles, and half-built working carts.", { type: "market", service: "cartwright" }),
  poi("carriage-works", "Royal Carriage Works", "iron-quarter", "A skilled workshop building sprung coaches, courier traps, and ceremonial wagons.", { type: "market", service: "carriage-wright" }),

  // Noble Rise (4; 1 service)
  poi("garden-court", "Garden Court", "noble-rise", "A quiet court of clipped yew, pale gravel, guarded doors, and discreet servants."),
  ...
  poi("rise-provisioner", "Noble Rise Provisioner", "noble-rise", "A precise shop supplying fine travel goods, household stores, and sealed gift baskets.", { type: "market", service: "general-store" }),

  // Citadel Ward (5; 1 service)
  ...
  poi("muster-court", "Royal Muster Court", "citadel-ward", "A broad drill court marked for companies, cavalry files, baggage, and inspection."),
  poi("iron-palace", "The Iron Palace", "citadel-ward", "The black-and-white seat of Whitemarch's crown, austere above the crowded wards."),
  poi("dragon-watch", "Dragon Watch Tower", "citadel-ward", "A high signal tower maintaining smoke, mirror, bell, and night-fire watches."),
  poi("citadel-sutler", "Citadel Sutler", "citadel-ward", "A licensed counter selling campaign necessities to guards, messengers, and departing patrols.", { type: "market", service: "sutler" }),

  // Caravan Ward (5; 3 services)
  poi("caravan-yard", "Caravan Yard", "caravan-ward", "A vast wagon court where incoming companies unhitch, count loads, and await customs."),
  poi("great-stable", "The Great Stable", "caravan-ward", "Long clean stable ranges with remount pens, tack rooms, fodder lofts, and ostlers.", { type: "stable", service: "stable" }),
  poi("farrier-lane", "Farrier Lane", "caravan-ward", "A hard-paved lane of shoeing stalls and animal doctors serving the road traffic.", { type: "smithy", service: "farrier" }),
  poi("caravan-tavern", "The Six Roads Inn", "caravan-ward", "A large travellers' inn with guarded yards, common tables, and multilingual notices.", { type: "tavern", service: "tavern" }),
  poi("customs-yard", "Customs Examination Yard", "caravan-ward", "A fenced yard where wagons are opened, assessed, sealed, or turned aside."),

  // Outer Works (5; no services)
  poi("gate-watchpost", "Gate Watchpost", "outer-works", "A fortified watch station coordinating patrols along the wall and approach roads."),
  poi("road-fort", "Crown Road Fort", "outer-works", "A compact road fort controlling the nearest high ground beyond the main gate."),
  poi("bridge-fort", "Whitewend Bridge Fort", "outer-works", "A low stone fort covering the bridge approaches, toll barriers, and river traffic."),
  poi("toll-hall", "Toll Hall", "outer-works", "A busy customs hall of permits, cargo tallies, axle fees, and disputed exemptions."),
...
function buildCapitalBlueprint() {
  ...
  if (Object.keys(tiles).length !== 469 || named.length !== 60 || services.length !== 22) {
