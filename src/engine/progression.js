...
const COMBAT_PROGRESSION_MODEL = "archetype";
const LEGACY_ARCHETYPE_FIELD = "sub" + "class";
const RETIRED_COMBAT_PROGRESSION_FIELDS = Object.freeze([
  "progression",
  "level",
...
export function stripCombatLegacyProgression(character, { forceCombat = false } = {}) {
  if (!character || (!forceCombat && usesLegacyCharacterProgression(character))) return character;
  if (forceCombat) character.progressionModel = COMBAT_PROGRESSION_MODEL;
  for (const key of RETIRED_COMBAT_PROGRESSION_FIELDS) {
...
function cloneCombatProjectionField(value) {
  if (Array.isArray(value)) return [...value];
  return value && typeof value === "object" ? { ...value } : value;
}

function synchronizeCombatPlayerProjection(character, projection) {
  if (!character || !projection) return;
  for (const key of ["profession", "archetype", "combatArchetypeId", "combatBaseStats", "attributes"]) {
    const value = character[key] ?? projection[key];
    if (value == null) continue;
    character[key] = cloneCombatProjectionField(value);
    projection[key] = cloneCombatProjectionField(value);
  }
  character.progressionModel = COMBAT_PROGRESSION_MODEL;
  projection.progressionModel = COMBAT_PROGRESSION_MODEL;
}
...
function retiredProfessionTreeLevel(progression, professionId) {
  const canonical = canonicalProfessionId(professionId);
  if (!canonical || !progression?.professionTree?.allocations) return 0;
  return Math.min(PROFESSION_LEVEL_CAP, Object.values(progression.professionTree.allocations).reduce((total, allocation) => (
    canonicalProfessionId(allocation?.professionId) === canonical ? total + 1 : total
  ), 0));
}

function persistedProfessionTrackLevel(progression, track) {
  return Math.min(PROFESSION_LEVEL_CAP, Math.max(
    rankTotal(track?.paths, PROFESSION_LEVEL_CAP),
    retiredProfessionTreeLevel(progression, track?.professionId),
  ));
}
...
export function professionProgressionLevel(value) {
  ...
  if (isStructuredProgression(progression)) {
    return Math.min(PROFESSION_LEVEL_CAP, progression.professions.reduce((sum, track) => (
      sum + persistedProfessionTrackLevel(progression, track)
    ), 0));
  }
...
function restoreStructuredSaveChoices(progression, source) {
  for (const track of progression.professions) {
    const saved = source.professions.find((entry) => canonicalProfessionId(entry.professionId) === track.professionId);
    if (!saved) continue;
    const levels = persistedProfessionTrackLevel(source, saved);
    track.branchChoices = { ...reachedProfessionBranchChoices(track.professionId, saved.branchChoices, levels) };
  ...
  const racialLevels = rankTotal(source.racial?.paths, RACIAL_LEVEL_CAP);
  progression.racial.branchChoices = { ...reachedRacialBranchChoices(
    progression.racial.raceId,
    source.racial?.branchChoices,
    racialLevels,
  ) };
  ...
  progression.racial.paths = allocatedRanks(racialCompiled, racialLevels);
  return syncCompatibility(progression);
}
...
function progressionAllocations(progression) {
  return {
    professions: progression.professions.map((track) => ({
      ...
      levels: persistedProfessionTrackLevel(progression, track), choices: track.choices,
      branchChoices: track.branchChoices,
    })),
...
function cloneProgression(progression) {
  const hasRetiredTree = Boolean(progression?.professionTree?.allocations);
  const base = { ...progression };
  delete base.professionTree;
  const cloned = {
    ...base,
    version: PROGRESSION_VERSION,
    ...
    professions: progression.professions.map((track) => {
      ...
      if (!hasRetiredTree) return copied;
      const compiled = compileProfessionTrack(copied.professionId, {
        specializationId: copied.specializationId,
        choices: copied.choices,
        branchChoices: copied.branchChoices,
      });
      copied.paths = allocatedRanks(compiled, persistedProfessionTrackLevel(progression, track));
      return copied;
    }),
  ...
  return syncCompatibility(cloned);
}
...
export function normalizeCharacterProgression(character, {
  ...
  if (!usesLegacyCharacterProgression(character)) return stripCombatLegacyProgression(character);
  const legacyKey = "sub" + "class";
  ...
  if (hadStructuredProgression && !explicitPlan && racialLevels == null && !signatureSpellId && !metamagicIds) {
    character.progression = restoreStructuredSaveChoices(createProgression({
      level: inferredLevel,
      professions: oldProgression.professions.map((track) => ({
        professionId: track.professionId, specializationId: track.specializationId,
        levels: persistedProfessionTrackLevel(oldProgression, track), choices: track.choices,
        branchChoices: track.branchChoices,
      })),
...
export function migrateProgressionState(state, { alignAuthoredAttributes = false } = {}) {
  ...
  const rootUsesCombatProgression = Boolean(
    state.character && (
  ...
  if (rootUsesCombatProgression && liveWanderer) synchronizeCombatPlayerProjection(state.character, liveWanderer);
  const migrate = (character, isPlayerProjection = false, forceCombat = false) => {
    if (forceCombat) return stripCombatLegacyProgression(character, { forceCombat: true });
  ...
  if (state.character) {
    ...
    migrate(state.character, true, rootUsesCombatProgression);
  }
  for (const [characterId, character] of Object.entries(state.world?.codex?.characters || {})) {
    ...
    migrate(character, playerProjection, rootUsesCombatProgression && playerProjection);
  ...
  for (const turn of state.turns || []) {
    ...
    const turnUsesCombatProgression = Boolean(turn?.char && (
      rootUsesCombatProgression
    ...
    if (turnUsesCombatProgression && turnWanderer) synchronizeCombatPlayerProjection(turn.char, turnWanderer);
    if (turn?.char) migrate(turn.char, true, turnUsesCombatProgression);
    for (const [characterId, character] of Object.entries(turn?.world?.codex?.characters || {})) {
      const playerProjection = characterId === "wanderer" || character?.id === "wanderer";
      migrate(character, playerProjection, turnUsesCombatProgression && playerProjection);
    }
  }
...
export function pendingProgressionChoices(value) {
  ...
  for (const track of progression.professions) {
    ...
    for (const row of compiled.levels.slice(0, persistedProfessionTrackLevel(progression, track))) {
  ...
  if (professionLevel < PROFESSION_LEVEL_CAP) {
    ...
    for (const professionId of Object.keys(PROFESSION_PROFILES)) {
      ...
      options.push(Object.freeze({
        ...
        description: track ? `Take the next ${professionName} level.` : `Begin a new ${professionName} multiclass.`,
        currentTrackLevel: track ? persistedProfessionTrackLevel(progression, track) : 0,
      }));
    }
  }
...
export function resolveProfessionChoice(value, { professionId, choiceId, optionId }) {
  rejectCombatLegacyProgressionMutation(value);
...
export function advanceProgression(character, xpGain) {
  if (character && !usesLegacyCharacterProgression(character)) {
    stripCombatLegacyProgression(character);
    return {
      character,
      beforeLevel: 0,
      afterLevel: 0,
...
export function projectCharacterProgression(state) {
  ...
  if (!usesLegacyCharacterProgression(character)) {
    ...
    synchronizeCombatPlayerProjection(projectedCharacter, projectedWanderer);
    stripCombatLegacyProgression(projectedCharacter);
    stripCombatLegacyProgression(projectedWanderer);
    return {
      ...state,
...
export function progressionSummary(character) {
  ...
  return {
    ...
    professions: progression.professions.map((track) => ({
      professionId: track.professionId, specializationId: track.specializationId,
      level: persistedProfessionTrackLevel(progression, track), paths: { ...track.paths },
