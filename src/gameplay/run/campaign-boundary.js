...
export function readReferenceGameplay(campaignState, { campaignId } = {}) {
  ...
  const expectedSeed = attempt === null
    ? null
    : `${lineageSeed}:tower-winter:${attempt}`;
  if (
    attempt === null
    || attempt < 1
    || lineageSeed === null
    || restored.state.runId !== `${identity}:tower-winter:${attempt}`
...
export function startReferenceGatekeeperTrial(
  ...
  const seed = `${lineageSeed}:tower-winter:${attempt}`;
  const runId = `${identity}:tower-winter:${attempt}`;
