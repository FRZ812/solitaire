// Canonical, reusable Tower of Winter combat kits.
//
...
const identities = [
  {
    ...
    design: "Plate, mail, arming sword, and shield define the kit; cold steel replaces the source character's arctic identity.",
  ...
  },
  {
    ...
    design: "A practical bow, layered leather, canvas, and field tools replace demon-specific trophies or personal vengeance.",
  ...
  },
  {
    ...
    design: "Workshop-built mechanisms and readable engineering replace ownership, genius-child, and clocktower lore.",
  ...
  },
  {
    ...
    design: "A battle-tested raider silhouette replaces crowns, royal heraldry, age, and northern kingship.",
  ...
  },
  {
    ...
    design: "A human innate caster and elemental focus replace the source character's reptilian body, scales, tail, and personal survival story.",
  ...
  },
  {
    ...
    design: "Functional covert gear replaces last-survivor lore, named schools, and a singular legendary assassin identity.",
  ...
  },
  {
    ...
    design: "Pact implements and anonymous remains replace an immortal witch's personal history and sovereign identity.",
  ...
  },
  {
    ...
    design: "A reusable field scholar replaces obsessive biography, personal age, and singular theorem lore.",
  ...
  },
  {
    ...
    design: "Neutral oath heraldry replaces exile, priestess, broken-halo, gendered, and institution-specific lore.",
  ...
  },
  {
    ...
    design: "Training discipline replaces a specific wanderer, biography, school grievance, and culturally singular personal title.",
  ...
  },
  {
    ...
    design: "A broad vampiric combat tradition replaces grief, isolation, named victims, and one immortal duelist's history.",
  ...
  },
  {
    ...
    design: "A configurable chassis replaces abandonment, a lost directive, and one machine's personal awakening narrative.",
...
export const TOW_ARCHETYPE_IDENTITIES = Object.freeze(identities.map((entry) => Object.freeze({
  ...entry,
...
const byCanonicalId = new Map(TOW_ARCHETYPE_IDENTITIES.map((entry) => [entry.id, entry]));
const byAnyId = new Map(TOW_ARCHETYPE_IDENTITIES.flatMap((entry) => [
  [entry.id, entry],
  [entry.legacyId, entry],
]));

export function getTowArchetypeIdentity(id) {
  return typeof id === "string" ? byAnyId.get(id) || null : null;
}

export function canonicalTowArchetypeId(id) {
  return getTowArchetypeIdentity(id)?.id || null;
}

export function legacyTowArchetypeId(id) {
  return getTowArchetypeIdentity(id)?.legacyId || null;
}

export function sameTowArchetype(left, right) {
  const a = canonicalTowArchetypeId(left);
  return Boolean(a && a === canonicalTowArchetypeId(right));
}

export const TOW_ARCHETYPE_ID_BY_LEGACY_ID = Object.freeze(Object.fromEntries(
  TOW_ARCHETYPE_IDENTITIES.map((entry) => [entry.legacyId, entry.id]),
));

export const TOW_ARCHETYPE_LEGACY_ID_BY_ID = Object.freeze(Object.fromEntries(
  TOW_ARCHETYPE_IDENTITIES.map((entry) => [entry.id, entry.legacyId]),
));

export function isCanonicalTowArchetypeId(id) {
