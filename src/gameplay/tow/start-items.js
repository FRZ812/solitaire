// Tower of Winter equipment grants used by the level-free character start.
//
// Campaign inventory remains the authority for ownership and equipment. This registry only
// answers what a worn, canonical Solitaire item contributes to a Tower of Winter fight. The
...
export function towStartItemGrants() {
...
export function towItemActorBonuses(itemIds = []) {
  ...
  for (const itemId of new Set(Array.isArray(itemIds) ? itemIds : [])) {
    const definition = getTowStartItemGrant(itemId);
...
export function describeTowItems(itemIds = []) {
  return (Array.isArray(itemIds) ? itemIds : []).map((itemId) => {
    const item = itemTemplate(itemId);
    const grant_ = getTowStartItemGrant(itemId);
    if (!item) return null;
    return {
      id: itemId,
      name: item.name,
...
export function invalidTowStartItemGrants() {
  const invalid = [];
  for (const definition of towStartItemGrants()) {
