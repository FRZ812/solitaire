// Pure enemy intent authority for solitaire-tow-v2.
//
...
export const TOW_AI_POLICY_REGISTRY_V2_VERSION = 1;
export const TOW_AI_PROFILE_VERSION_V2 = 1;

export const TOW_AI_PROFILE_IDS_V2 = Object.freeze([
  "knight",
  "ranger",
  "artificer",
  "berserker",
...
function policyReason(value) {
  ...
  const kit = TOW_DEFAULT_ABILITY_KITS_V2[value.profileId];
  const actionKit = kit?.filter((id) => getTowAbilityRulesV2(id).action.lane !== "reaction");
...
function createPolicy(profileId) {
  const policy = {
    version: TOW_AI_POLICY_REGISTRY_V2_VERSION,
    rulesetId: TOW_ABILITY_RULESET_V2_ID,
    profileId,
    profileVersion: TOW_AI_PROFILE_VERSION_V2,
    policyId: `${profileId}-v${TOW_AI_PROFILE_VERSION_V2}`,
    lanePriority: ["quick", "main"],
    rolePriority: [...ROLE_PRIORITIES[profileId]],
    abilityPriority: TOW_DEFAULT_ABILITY_KITS_V2[profileId]
      .filter((id) => getTowAbilityRulesV2(id).action.lane !== "reaction"),
    randomness: "none",
  };
...
export const TOW_AI_POLICY_REGISTRY_V2 = deepFreeze(Object.fromEntries(
  TOW_AI_PROFILE_IDS_V2.map((profileId) => {
    const policy = createPolicy(profileId);
    return [policy.policyId, policy];
  }),
));

export function calculateTowAiPolicyRegistryV2Checksum() {
  return `fnv1a32:${fnv1a32(stableSerialize({
    version: TOW_AI_POLICY_REGISTRY_V2_VERSION,
    rulesetId: TOW_ABILITY_RULESET_V2_ID,
    policies: Object.values(TOW_AI_POLICY_REGISTRY_V2),
  }))}`;
}
...
export const TOW_AI_POLICY_REGISTRY_V2_CHECKSUM = "fnv1a32:9bcc646d";

if (calculateTowAiPolicyRegistryV2Checksum() !== TOW_AI_POLICY_REGISTRY_V2_CHECKSUM) {
  throw new TypeError("tow-ai-policy-v2-checksum-drift");
}

export function validateTowAiPolicyV2(value) {
  const reason = policyReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isTowAiPolicyV2(value) {
  return validateTowAiPolicyV2(value).ok;
}

export function getTowAiPolicyV2(profileId, profileVersion) {
  if (!rulesIdentifier(profileId) || !positiveSafeInteger(profileVersion)) return null;
  return TOW_AI_POLICY_REGISTRY_V2[`${profileId}-v${profileVersion}`] ?? null;
}
...
function intentReason(value) {
  ...
  if (!rulesIdentifier(value.abilityId)
    ...
    || !Object.hasOwn(TOW_AI_POLICY_REGISTRY_V2, value.policyId)
  ...
  if (value.targetLock.abilityId !== value.abilityId
    ...
    || value.targetLock.rulesetId !== TOW_ABILITY_RULESET_V2_ID) {
...
export function validateTowAiIntentV2(value) {
  const reason = intentReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isTowAiIntentV2(value) {
  return validateTowAiIntentV2(value).ok;
}
...
function checkedState(state) {
  return !state || typeof state !== "object" || Array.isArray(state)
    || state.version !== TOW_ABILITY_RULES_V2_VERSION
    || state.rulesetId !== TOW_ABILITY_RULESET_V2_ID
...
function policyForActor(actor) {
  ...
  return getTowAiPolicyV2(actor.aiProfile.id, actor.aiProfile.version);
}
...
function statusMagnitude(state, actorId, statusId) {
  return towStatusMagnitudeV2(state.statuses, actorId, statusId);
}
...
function candidateMetrics(state, actorId, policy, ability, commit) {
  ...
  const roles = getTowAbilityRolesV2(ability.id) ?? [];
...
function forcedTargetProjection(state, actorId, ability, anchored) {
  ...
  return resolveTowForcedTargetV2(state.statuses, { actorId, validActorIds });
}
...
function abilityCandidates(state, actor, policy, loadout) {
  ...
  const access = canUseTowAbilityV2(state.economy, {
    actorId: actor.id,
    abilityId: ability.id,
  });
  if (!access.ok) return { ok: true, reason: null, candidates: [] };
  const gate = adjudicateTowStatusActionV2(state.statuses, {
    actorId: actor.id,
 ...
 */
export function declareTowAiIntentV2(state, input) {
...
export function evaluateTowAiIntentV2(state, input) {
  const stateError = checkedState(state);
  ...
  const structural = validateTowAiIntentV2(input.intent);
  ...
  const definition = getTowAbilityRulesV2(input.intent.abilityId);
 ...
 */
export function redeclareTowAiIntentV2(state, input) {
...
if (TOW_ABILITY_RULES_V2_VERSION !== 2) {
  throw new TypeError("tow-ai-v2-rules-version-drift");
}
