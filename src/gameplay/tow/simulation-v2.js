// Pure deterministic gauntlet and reachability harness for solitaire-tow-v2.
//
...
import { canUseTowAbilityV2 } from "./action-economy-v2.js";
import {
  dispatchTowCommandV2,
} from "./commands-v2.js";
import { TOW_DAMAGE_POLICY_V2 } from "./damage-v2.js";
import {
...
import { resolveTowForcedTargetV2 } from "./status-runtime-v2.js";

export const TOW_SIMULATION_V2_VERSION = 1;
export const TOW_SIMULATION_RANK_TIERS_V2 = Object.freeze(["rank-1", "mid", "max"]);
export const TOW_SIMULATION_CASE_KINDS_V2 = Object.freeze([
  "solo-mirror",
  "mixed-3v3",
  "mixed-5v5",
  "ability-probe",
]);
export const TOW_SIMULATION_STOP_REASONS_V2 = Object.freeze([
  "terminal",
  "round-bound",
  "command-bound",
  "event-bound",
...
export const TOW_SIMULATION_BOUNDS_V2 = deepFreeze({
  maxRounds: 6,
  maxCommands: 512,
  maxEvents: 20_000,
  maxReactions: 128,
  maxAiDrawsPerStep: MAX_TOW_AI_STEP_RANDOM_DRAWS_V2,
});
...
function kitLoadout(profileId, tier) {
  const ids = TOW_DEFAULT_ABILITY_KITS_V2[profileId];
  if (!ids) throw new TypeError("unknown-simulation-v2-profile");
  return ids.map((id) => ({
    id,
    rank: rankForTier(getTowAbilityRulesV2(id), tier),
  }));
}
...
function spec(input) {
  const candidate = deepFreeze({
    ...
    maxRounds: input.maxRounds ?? TOW_SIMULATION_BOUNDS_V2.maxRounds,
    maxCommands: input.maxCommands ?? TOW_SIMULATION_BOUNDS_V2.maxCommands,
    maxEvents: input.maxEvents ?? TOW_SIMULATION_BOUNDS_V2.maxEvents,
    maxReactions: input.maxReactions ?? TOW_SIMULATION_BOUNDS_V2.maxReactions,
    genesis: input.genesis,
  });
  const validation = validateTowSimulationCaseV2(candidate);
  if (!validation.ok) throw new TypeError(validation.reason);
  return candidate;
}

function soloMirrorCases() {
  return Object.keys(TOW_DEFAULT_ABILITY_KITS_V2).flatMap((profileId) => (
    TOW_SIMULATION_RANK_TIERS_V2.map((rankTier, tierIndex) => {
      const actors = [
        createActor({
          id: `player:${profileId}`,
          side: "player",
...
function mixedCases() {
  const profiles = Object.keys(TOW_DEFAULT_ABILITY_KITS_V2);
  return [
    [3, "mid", false],
    [3, "mid", true],
    [5, "max", false],
...
function abilityProbeCases() {
  return TOW_ABILITY_CATALOG_V2_LIST.map((definition, index) => {
    const abilityId = definition.id;
    const profileId = Object.entries(TOW_DEFAULT_ABILITY_KITS_V2)
...
export const TOW_SIMULATION_GAUNTLET_CASES_V2 = deepFreeze([
  ...soloMirrorCases(),
  ...mixedCases(),
  ...abilityProbeCases(),
]);

export const TOW_SIMULATION_REQUIRED_COVERAGE_V2 = deepFreeze({
  abilityIds: sortedUnique(TOW_ABILITY_CATALOG_V2_LIST.map(({ id }) => id)),
  statusIds: sortedUnique(TOW_ABILITY_STATUS_LIST_V2.map(({ id }) => id)),
  zoneIds: sortedUnique(TOW_ABILITY_ZONE_LIST_V2.map(({ id }) => id)),
  profileIds: sortedUnique(Object.keys(TOW_DEFAULT_ABILITY_KITS_V2)),
  rankTiers: [...TOW_SIMULATION_RANK_TIERS_V2],
});

export function validateTowSimulationCaseV2(value) {
  let reason = null;
  try {
    if (!exactKeys(value, SPEC_KEYS)) reason = "invalid-simulation-v2-case-shape";
    else if (!identifier(value.id)
      || !TOW_SIMULATION_CASE_KINDS_V2.includes(value.kind)
      || !TOW_SIMULATION_RANK_TIERS_V2.includes(value.rankTier)
      || typeof value.sideSwapped !== "boolean"
      || typeof value.expectedHoleyFormation !== "boolean"
      || (value.coverageAbilityId !== null
        && !TOW_ABILITY_CATALOG_V2_LIST.some(({ id }) => id === value.coverageAbilityId))) {
      reason = "invalid-simulation-v2-case-identity";
    } else if (!positiveSafeInteger(value.maxRounds)
      || value.maxRounds > TOW_SIMULATION_BOUNDS_V2.maxRounds
      || !positiveSafeInteger(value.maxCommands)
      || value.maxCommands > TOW_SIMULATION_BOUNDS_V2.maxCommands
      || !positiveSafeInteger(value.maxEvents)
      || value.maxEvents > TOW_SIMULATION_BOUNDS_V2.maxEvents
      || !positiveSafeInteger(value.maxReactions)
      || value.maxReactions > TOW_SIMULATION_BOUNDS_V2.maxReactions) {
...
export function towSimulationRandomDrawsV2(input) {
  if (!exactKeys(input, DRAW_INPUT_KEYS)
    ...
    || input.count < 0
    || input.count > TOW_SIMULATION_BOUNDS_V2.maxAiDrawsPerStep) {
...
function dispatch(session, scenarioId, command) {
  return dispatchTowCommandV2(session, commandInput(session, scenarioId, command));
}
...
function chosenTarget(state, ability, actorId, { preferSelf = false } = {}) {
  ...
  const forced = resolveTowForcedTargetV2(state.statuses, { actorId, validActorIds });
...
function verifySession(session) {
  ...
  const expectedStateChecksum = towEncounterStateChecksumV2(session.encounter);
  const replayStateChecksum = towEncounterStateChecksumV2(replay.encounter);
  const decodedStateChecksum = towEncounterStateChecksumV2(decoded.session.encounter);
...
function statusIdsFromZoneEvents(events) {
  const zoneById = Object.fromEntries(TOW_ABILITY_ZONE_LIST_V2.map((zone) => [zone.id, zone]));
...
function actorKitTier(actor) {
  ...
  const tier = TOW_SIMULATION_RANK_TIERS_V2.find((candidate) => (
    TOW_DEFAULT_ABILITY_KITS_V2[profileId].every((id) => (
      ranks[id] === rankForTier(getTowAbilityRulesV2(id), candidate)
    ))
  ));
  return tier === undefined ? null : `${profileId}:${tier}`;
}

export function collectTowSimulationTelemetryV2(session) {
  const valid = validateTowSessionV2(session);
...
function success(caseSpec, stopReason, session, verifyPersistence) {
  const telemetry = collectTowSimulationTelemetryV2(session);
  const verification = verifyPersistence
    ...
    : deepFreeze({
      ...
      reason: null,
      sessionChecksum: session.checksum,
      stateChecksum: towEncounterStateChecksumV2(session.encounter),
...
function runAbilityProbe(caseSpec, verifyPersistence) {
  ...
  const definition = getTowAbilityRulesV2(caseSpec.coverageAbilityId);
  ...
  if (ability.action.lane !== "reaction") {
    ...
    const acted = dispatchRequired(caseSpec, session, {
      ...
      randomDraws: towSimulationRandomDrawsV2({
  ...
  const hostileDefinition = getTowAbilityRulesV2("arctic-strike");
...
function actorProfileId(actor) {
  ...
  return Object.entries(TOW_DEFAULT_ABILITY_KITS_V2)
...
function orderedLaneCandidates(session, actor, lane) {
  const profileId = actorProfileId(actor);
  const authoredOrder = profileId === null ? [] : TOW_DEFAULT_ABILITY_KITS_V2[profileId];
  return actor.loadout.flatMap((loadout) => {
    const definition = getTowAbilityRulesV2(loadout.id);
    const ability = abilityRulesV2AtRank(definition, loadout.rank);
    if (ability.action.lane !== lane) return [];
    const available = canUseTowAbilityV2(session.encounter.economy, {
      actorId: actor.id,
      abilityId: ability.id,
    });
...
function armHumanReaction(caseSpec, session, actor) {
  ...
  const reaction = actor.loadout.flatMap((loadout) => {
    const ability = abilityRulesV2AtRank(getTowAbilityRulesV2(loadout.id), loadout.rank);
 ...
 */
export function runTowSimulationCaseV2(input) {
  ...
  const validation = validateTowSimulationCaseV2(input.case);
...
export function aggregateTowSimulationTelemetryV2(results) {
...
export function calculateTowSimulationGauntletV2Checksum() {
  return `fnv1a64:${gameplayChecksum({
    version: TOW_SIMULATION_V2_VERSION,
    bounds: TOW_SIMULATION_BOUNDS_V2,
    cases: TOW_SIMULATION_GAUNTLET_CASES_V2,
    requiredCoverage: TOW_SIMULATION_REQUIRED_COVERAGE_V2,
  })}`;
}

export const TOW_SIMULATION_GAUNTLET_V2_CHECKSUM = "fnv1a64:943eff406e540c59";

if (TOW_ABILITY_RULES_V2_VERSION !== 2
  || TOW_ABILITY_CATALOG_V2_LIST.length !== 60
  || TOW_ABILITY_STATUS_LIST_V2.length !== 30
  || TOW_ABILITY_ZONE_LIST_V2.length !== 7
  || calculateTowSimulationGauntletV2Checksum() !== TOW_SIMULATION_GAUNTLET_V2_CHECKSUM) {
  throw new TypeError("tow-simulation-v2-upstream-drift");
}
