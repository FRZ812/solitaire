...
import manifest from "./fixtures/v12-verifier-manifest.json";

const root = fileURLToPath(new URL("../../../", import.meta.url));
const provenance = "// Frozen verifier-only Tower v1.2 semantics from deployed commit d925c35.\n"
...
describe("the frozen deployed-v1.2 verifier graph", () => {
  it("matches the complete deployed source closure through an external Git oracle", () => {
    expect(manifest).toMatchObject({
      version: 2,
      sourceCommit: "d925c35220a17827a32ba5de1f330efd483ce505",
      purpose: "verifier-only Tower v1.2 replay graph",
    });
  ...
  it("is unreachable from the playable runtime registry", () => {
    ...
    expect(runtime).not.toContain("-v12.js");
    expect(currentCommands).not.toContain("-v12.js");
    expect(currentReplay).not.toContain("-v12.js");
    expect(runtime).not.toContain("verifyRetiredTowV12Session");
  });
});
