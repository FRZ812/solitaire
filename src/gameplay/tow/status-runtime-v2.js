// Executable status authority for the opt-in solitaire-tow-v2 ruleset.
//
...
export const TOW_STATUS_RUNTIME_V2_VERSION = 1;
export const TOW_STATUS_BOUNDARIES_V2 = Object.freeze([
  "round-start",
  "round-end",
  "source-turn-start",
  "source-turn-end",
...
export const TOW_STATUS_BEHAVIOR_POLICY_V2 = deepFreeze({
...
const POLICY_IDS = Object.keys(TOW_STATUS_BEHAVIOR_POLICY_V2).sort(compareIdentifiers);
const REGISTRY_IDS = Object.keys(TOW_ABILITY_STATUSES_V2).sort(compareIdentifiers);
if (POLICY_IDS.length !== REGISTRY_IDS.length
  || !POLICY_IDS.every((id, index) => id === REGISTRY_IDS[index])
  || !POLICY_IDS.every((id) => (
    TOW_STATUS_BEHAVIOR_POLICY_V2[id].behavior === TOW_ABILITY_STATUSES_V2[id].behavior
  ))) {
...
export function calculateTowStatusPolicyV2Checksum() {
  return `fnv1a32:${fnv1a32(stableSerialize({
    ...
    definitions: TOW_ABILITY_STATUS_LIST_V2,
    policies: TOW_STATUS_BEHAVIOR_POLICY_V2,
  }))}`;
}

// Literal is checked by tests so policy drift requires an intentional review.
export const TOW_STATUS_POLICY_V2_CHECKSUM = "fnv1a32:bcab7c74";
...
function stateReason(value) {
  ...
  if (value.version !== TOW_ABILITY_RULES_V2_VERSION
    ...
    || value.runtimeVersion !== TOW_STATUS_RUNTIME_V2_VERSION) {
  ...
  for (const [actorId, records] of Object.entries(value.actors)) {
    ...
    for (let index = 0; index < records.length; index += 1) {
      ...
      const definition = TOW_ABILITY_STATUSES_V2[record.id];
...
export function validateTowStatusRuntimeV2(value) {
  const reason = stateReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isTowStatusRuntimeV2(value) {
  return stateReason(value) === null;
}

export function createTowStatusRuntimeV2(input) {
...
export function towStatusMagnitudeV2(state, actorId, statusId) {
  const reason = stateReason(state);
  ...
  if (typeof statusId !== "string" || !Object.hasOwn(TOW_ABILITY_STATUSES_V2, statusId)) {
...
function mutationReason(state, input) {
  if (!exactKeys(input, MUTATION_KEYS)
    ...
    || !Object.hasOwn(TOW_ABILITY_STATUSES_V2, input.statusId)
  ...
  const definition = TOW_ABILITY_STATUSES_V2[input.statusId];
...
export function mutateTowStatusV2(state, input) {
...
function validResolverRecord(statusId, record) {
  ...
  const definition = TOW_ABILITY_STATUSES_V2[statusId];
...
function createStatusResolverV2(statusId) {
  const statusPolicy = TOW_STATUS_BEHAVIOR_POLICY_V2[statusId];
  return (context) => {
    if (!exactKeys(context, RESOLVER_CONTEXT_KEYS)
      || !TOW_STATUS_BOUNDARIES_V2.includes(context.boundary)
...
export const TOW_STATUS_RUNTIME_RESOLVERS_V2 = defineStatusRuntimeResolversV2(
  TOW_ABILITY_STATUSES_V2,
  {
...
export function advanceTowStatusBoundaryV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
  if (!exactKeys(input, BOUNDARY_KEYS)
    || !TOW_STATUS_BOUNDARIES_V2.includes(input.boundary)
  ...
  for (const [holderActorId, records] of Object.entries(current.actors)) {
    ...
    for (const original of records) {
      const definition = TOW_ABILITY_STATUSES_V2[original.id];
      let record = cloneRecord(original);
      if (boundaryAppliesToRecord(input.boundary, input.actorId, holderActorId, record)) {
        const resolved = TOW_STATUS_RUNTIME_RESOLVERS_V2[record.id]({
          boundary: input.boundary,
          holderActorId,
          record,
        });
...
export function adjudicateTowStatusActionV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
  ...
  const blocking = current.actors[input.actorId].find((record) => (
    TOW_STATUS_BEHAVIOR_POLICY_V2[record.id].actionLockLanes.includes(input.lane)
  ));
  if (!blocking) {
...
export function resolveTowForcedTargetV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function resolveTowMovementAllowanceV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
  ...
  const penalty = blocked ? input.requestedCells : Math.ceil(
    limp / TOW_STATUS_BEHAVIOR_POLICY_V2.limp.movement.magnitudePerCell,
  );
...
export function towStatusCombatModifiersV2(state, actorId) {
  const reason = stateReason(state);
  if (reason !== null) throw new TypeError(reason);
  if (!actorIdentifier(actorId) || !Object.hasOwn(state.actors, actorId)) {
    throw new TypeError("unknown-status-runtime-v2-actor");
  ...
  for (const record of state.actors[actorId]) {
    const row = TOW_STATUS_BEHAVIOR_POLICY_V2[record.id];
...
export function resolveTowDirectHitStatusesV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  ...
  if (input.landed) {
    const attackerHooks = current.actors[input.attackerActorId]
      .filter((record) => (
        TOW_STATUS_BEHAVIOR_POLICY_V2[record.id].afterDirectHit.applyStatus !== null
      ))
      .map(cloneRecord);
    for (const record of attackerHooks) {
      const hook = TOW_STATUS_BEHAVIOR_POLICY_V2[record.id].afterDirectHit;
    ...
    const spent = current.actors[input.defenderActorId]
      .filter((record) => (
        TOW_STATUS_BEHAVIOR_POLICY_V2[record.id].afterDirectHit.consumeDefender > 0
      ))
      .map((record) => record.id);
    for (const statusId of spent) {
      const amount = TOW_STATUS_BEHAVIOR_POLICY_V2[statusId].afterDirectHit.consumeDefender;
      const mutation = mutateUnchecked(current, {
        actorId: input.defenderActorId,
        operation: "subtract",
        sourceActorId: null,
