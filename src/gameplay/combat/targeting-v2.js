// Pure targeting for rank-resolved solitaire-combat-v2 abilities.
//
...
export const COMBAT_TARGET_COMMIT_V2_VERSION = 1;
export const COMBAT_TARGET_LOCK_V2_VERSION = 1;
export const COMBAT_FORMATION_SIDES_V2 = Object.freeze(["player", "enemy"]);
...
function isRankResolvedAbilityV2(value) {
  return Boolean(
    ...
    && typeof value === "object"
    && !Array.isArray(value)
    && value.version === COMBAT_ABILITY_RULES_V2_VERSION
...
function assertTargetState(state) {
  ...
  try {
    for (const side of COMBAT_FORMATION_SIDES_V2) {
...
function actorCell(state, id) {
  const actor = state.actors[id];
  if (!actor || !COMBAT_FORMATION_SIDES_V2.includes(actor.side)) return null;
...
export function lockAbilityTargetV2(state, ability, casterId, anchor) {
  ...
  const lock = {
    version: COMBAT_TARGET_LOCK_V2_VERSION,
...
export function isAbilityTargetLockV2(value) {
  if (!exactKeys(value, LOCK_KEYS)
    || value.version !== COMBAT_TARGET_LOCK_V2_VERSION
    || value.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID
    ...
    || !COMBAT_FORMATION_SIDES_V2.includes(value.anchor.side)
...
export function commitAbilityTargetsV2(state, ability, lock) {
  ...
  const result = {
    ok: true,
    reason: null,
    version: COMBAT_TARGET_COMMIT_V2_VERSION,
    rulesetId: COMBAT_ABILITY_RULESET_V2_ID,
    abilityId: ability.id,
    rank: ability.rank,
    casterId,
