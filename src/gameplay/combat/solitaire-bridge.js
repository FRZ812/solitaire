// Solitaire's world → Solitaire combat actors.
//
// The kernel is a faithful port of Solitaire combat's mechanics, but the fights it has to
...
import { admitCombatEncounter } from "./admission.js";
...
export const PROVISIONAL_BRIDGE_POLICY = Object.freeze({
  // Solitaire combat actors have one attack number; Solitaire weapons have a damage band.
  ...
  // of taste. Solitaire combat skills scale off ATK as a percentage — Strike is "100% of ATK"
  ...
  // DEF in Solitaire combat is what Block turns into shield and what Tenacity raises. It is
  ...
  // someone in cloth; Solitaire combat's DEF is a stat every actor carries — the Arctic
...
export function combatArchetypeForEnemy(enemy = {}) {
  const explicit = getStartingArchetype(enemy.combatArchetypeId)
...
function enemyCombatBuild(enemy) {
  const archetype = combatArchetypeForEnemy(enemy);
...
function resolveSnapshot(character, fallbackMind = 0, {
  ...
  const previousMaxBonus = Math.min(
    nonNegativeInt(character?.combatResolveMaxBonus),
...
/**
 * Build the Solitaire combat player actor for a Solitaire character.
 *
 * @param {object} character
 * @param {object} codex world codex, for equipped gear
 * @param {{id?: string}} options
 */
export function combatPlayerFromCharacter(character, codex = {}, { id = "player" } = {}) {
  const stats = deriveCombatStats(character, codex);
  const itemBonus = combatItemActorBonuses(activeCombatItemIds(character, codex));
  const sourceBase = character.progressionModel === "archetype" && character.combatBaseStats;
  if (sourceBase
  ...
  // behind it, because a Solitaire combat skill scales off ATK alone.
...
/**
 * Build a Solitaire combat enemy actor from a bestiary entry.
 *
 ...
 */
export function combatEnemyFromBestiary(enemy, { id } = {}) {
  ...
  const identity = enemyCombatBuild(enemy);
...
/**
 * Whether a Solitaire encounter can run on the Solitaire combat kernel as it stands.
 *
 ...
 *
 * So it delegates. `admitCombatEncounter` decides, this reports the first objective blocker,
 * and there is one answer to the question rather than two that agree by luck.
 */
export function combatEncounterSupport({ character, party, enemies } = {}) {
  const admission = admitCombatEncounter({ character, party: party || [], enemies });
