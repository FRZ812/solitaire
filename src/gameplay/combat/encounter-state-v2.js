// Canonical genesis and composite state scaffold for solitaire-combat-v2.
//
...
import {
  COMBAT_ACTION_ECONOMY_POLICY_V2,
  createCombatActionEconomyV2,
  validateCombatActionEconomyV2,
} from "./action-economy-v2.js";
import {
  COMBAT_ABILITY_CATALOG_V2_CHECKSUM,
...
import {
  COMBAT_AI_POLICY_REGISTRY_V2_CHECKSUM,
  COMBAT_AI_POLICY_REGISTRY_V2_VERSION,
  getCombatAiPolicyV2,
  isCombatAiIntentV2,
} from "./ai-v2.js";
import {
...
import {
  COMBAT_STATUS_BEHAVIOR_POLICY_V2,
  COMBAT_STATUS_POLICY_V2_CHECKSUM,
  COMBAT_STATUS_RUNTIME_V2_VERSION,
  createCombatStatusRuntimeV2,
  validateCombatStatusRuntimeV2,
} from "./status-runtime-v2.js";
import {
  COMBAT_TARGET_LOCK_V2_VERSION,
  isAbilityTargetLockV2,
} from "./targeting-v2.js";
import {
  COMBAT_ZONE_RUNTIME_POLICY_V2,
  COMBAT_ZONE_RUNTIME_V2_VERSION,
  createCombatZoneStateV2,
  validateCombatZoneStateV2,
} from "./zones-v2.js";

export const PINNED_COMBAT_ABILITY_CATALOG_V2_CHECKSUM = "fnv1a32:8a8adfc6";
export const COMBAT_ENCOUNTER_POLICY_V2_ID = "solitaire-combat-v2-encounter-policy-v1";
export const COMBAT_ENCOUNTER_SCHEDULER_V2_VERSION = 1;

if (COMBAT_ABILITY_CATALOG_V2_CHECKSUM !== PINNED_COMBAT_ABILITY_CATALOG_V2_CHECKSUM) {
  throw new TypeError("combat-v2-catalog-checksum-drift");
}
...
export const COMBAT_ENCOUNTER_EXECUTION_POLICY_V2 = deepFreeze({
  ...
  ai: {
    registryChecksum: COMBAT_AI_POLICY_REGISTRY_V2_CHECKSUM,
    registryVersion: COMBAT_AI_POLICY_REGISTRY_V2_VERSION,
...
export const COMBAT_ENCOUNTER_POLICY_V2 = deepFreeze({
  id: COMBAT_ENCOUNTER_POLICY_V2_ID,
  catalogue: {
    checksum: PINNED_COMBAT_ABILITY_CATALOG_V2_CHECKSUM,
  ...
  actionEconomy: COMBAT_ACTION_ECONOMY_POLICY_V2,
  execution: COMBAT_ENCOUNTER_EXECUTION_POLICY_V2,
  damage: {
    policyChecksum: COMBAT_DAMAGE_POLICY_V2_CHECKSUM,
    policy: COMBAT_DAMAGE_POLICY_V2,
  },
  targeting: {
    lockVersion: COMBAT_TARGET_LOCK_V2_VERSION,
    reactionLockAtomicity: "iff-armed-reaction",
  },
  statuses: {
    runtimeVersion: COMBAT_STATUS_RUNTIME_V2_VERSION,
    policyChecksum: COMBAT_STATUS_POLICY_V2_CHECKSUM,
    behaviorPolicy: COMBAT_STATUS_BEHAVIOR_POLICY_V2,
  },
  zones: {
    runtimeVersion: COMBAT_ZONE_RUNTIME_V2_VERSION,
    runtimePolicy: COMBAT_ZONE_RUNTIME_POLICY_V2,
  },
  ai: {
    registryVersion: COMBAT_AI_POLICY_REGISTRY_V2_VERSION,
    registryChecksum: COMBAT_AI_POLICY_REGISTRY_V2_CHECKSUM,
...
function actorMapReason(actors, rosters) {
  ...
  for (const [id, actor] of Object.entries(actors)) {
    if (actor.id !== id || !isCombatActorV2(actor)) return "invalid-encounter-v2-actor";
...
function economyReason(economy, actors) {
  const validation = validateCombatActionEconomyV2(economy);
...
function statusesReason(statuses, actors) {
  const validation = validateCombatStatusRuntimeV2(statuses);
...
function zonesReason(zones, actors) {
  const validation = validateCombatZoneStateV2(zones);
...
function schedulerReason(scheduler, economy, actors, rosters) {
  if (!exactKeys(scheduler, SCHEDULER_KEYS)
    || scheduler.version !== COMBAT_ENCOUNTER_SCHEDULER_V2_VERSION
...
function intentsReason(intents, intentSequence, actors) {
  ...
  for (const [actorId, intent] of Object.entries(intents)) {
    ...
    const policy = actor?.aiProfile === null || actor?.aiProfile === undefined
      ? null
      : getCombatAiPolicyV2(actor.aiProfile.id, actor.aiProfile.version);
    const loadout = actor?.loadout.find(({ id }) => id === intent?.abilityId);
    if (!actor || actor.controller !== "ai" || actor.hp <= 0
      || !policy || !isCombatAiIntentV2(intent)
...
function stateReason(value) {
  ...
  if (value.catalogChecksum !== PINNED_COMBAT_ABILITY_CATALOG_V2_CHECKSUM) {
    return "invalid-encounter-state-v2-catalog-checksum";
  }
  if (value.aiPolicyChecksum !== COMBAT_AI_POLICY_REGISTRY_V2_CHECKSUM) {
    return "invalid-encounter-state-v2-ai-policy-checksum";
  }
  if (!sameData(value.policy, COMBAT_ENCOUNTER_POLICY_V2)) {
  ...
  if (Object.values(value.actors).some((actor) => actor.controller === "ai"
    && getCombatAiPolicyV2(actor.aiProfile.id, actor.aiProfile.version) === null)) {
...
export function validateCombatEncounterStateV2(value) {
  const reason = stateReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isCombatEncounterStateV2(value) {
  return validateCombatEncounterStateV2(value).ok;
}

/** Snapshot the sole deterministic owner-priority order for a new round. */
export function combatEncounterRoundOrderV2(value) {
  const validation = validateCombatEncounterStateV2(value);
...
export function defineCombatEncounterStateV2(value) {
  const validation = validateCombatEncounterStateV2(value);
  if (!validation.ok) throw new TypeError(validation.reason);
...
function genesisActors(inputActors, rosters) {
  ...
  if (entries.some(([id, actor]) => !actorIdentifier(id) || !isCombatActorV2(actor))
...
export function createCombatEncounterGenesisV2(input) {
  if (!exactKeys(input, GENESIS_KEYS)) {
    return result(false, "invalid-encounter-genesis-v2-input");
  }
  if (input.catalogChecksum !== PINNED_COMBAT_ABILITY_CATALOG_V2_CHECKSUM) {
    return result(false, "invalid-encounter-genesis-v2-catalog-checksum");
  }
  if (input.aiPolicyChecksum !== COMBAT_AI_POLICY_REGISTRY_V2_CHECKSUM) {
    return result(false, "invalid-encounter-genesis-v2-ai-policy-checksum");
  }
  if (input.policyId !== COMBAT_ENCOUNTER_POLICY_V2_ID) {
  ...
  const actorIds = Object.keys(actors);
  const economy = createCombatActionEconomyV2({
  ...
  const statuses = createCombatStatusRuntimeV2({ actorIds });
  if (!statuses.ok) return result(false, statuses.reason);
  const zones = createCombatZoneStateV2({ zones: [] });
  ...
  const state = {
    ...
    policy: cloneData(COMBAT_ENCOUNTER_POLICY_V2),
    rosters: {
