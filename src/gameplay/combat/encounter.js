// The Solitaire combat encounter loop.
//
...
import { createCombatActor, isCombatActor } from "../kernel/combat-actor.js";
import { resolveAttack } from "../kernel/combat-damage.js";
...
import { reflowCombatFormations } from "./movement.js";
...
export const COMBAT_ENCOUNTER_VERSION = 1;
...
export function createCombatEncounter({
  ...
  const playerActor = createCombatActor({ ...player, side: "player" });
  ...
  const allyActors = allies.map((ally) => {
    ...
    const actor = createCombatActor({ ...actorFields, side: "player" });
  ...
  const enemyActors = enemies.map((enemy) => {
    ...
    const actor = createCombatActor({ ...actorFields, side: "enemy" });
    if (enemyBuild) {
  ...
  const base = {
    version: COMBAT_ENCOUNTER_VERSION,
    phase: "player",
    round: 1,
    sequence: 0,
    rng: createRng(seed),
    ...
    intentRng: createRng(intentSeed ?? `${seed}::combat-stream::intent::v1`),
...
function reflowRoundFormations(state) {
  const reflowed = reflowCombatFormations(state);
...
function declareRoundIntents(state) {
  ...
  for (const enemyId of state.enemyIds) {
    ...
    if (held && isCombatIntent(held)) continue;
    const declared = declareCombatIntent({
      schedule,
      declarationIndex: 0,
...
function advanceEnemyIntent(state, enemyId) {
  ...
  const advanced = advanceCombatIntent({
    schedule,
    intent: held,
    targetId: state.playerId,
...
export function isCombatEncounter(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  if (value.version !== COMBAT_ENCOUNTER_VERSION) return false;
  ...
  if (!isKeyedByEnemies(value.intents, value.enemyIds, isCombatIntent)) return false;
  ...
  return actorIds.every((id) => isCombatActor(value.actors[id]));
}
