// Deterministic composite encounter reducer for solitaire-combat-v2.
//
...
import {
  applyCombatActionBudgetDeltaV2,
  applyCombatResolveDeltaV2,
  armCombatReactionV2,
  beginCombatActionRoundV2,
  beginCombatActorTurnV2,
  canUseCombatAbilityV2,
  commitCombatAbilityActionV2,
  endCombatActionRoundV2,
  endCombatActorTurnV2,
  expireCombatReactionV2,
  triggerCombatReactionV2,
...
import {
  ...
  getCombatStatusRulesV2,
} from "./ability-catalog-v2.js";
import {
  COMBAT_ACTOR_SCALAR_MAX_V2,
  defineCombatActorV2,
} from "./actor-v2.js";
import {
  declareCombatAiIntentV2,
  evaluateCombatAiIntentV2,
  redeclareCombatAiIntentV2,
} from "./ai-v2.js";
import {
  COMBAT_DAMAGE_MAX_V2,
  COMBAT_DAMAGE_POLICY_V2,
  resolveCombatDamageV2,
} from "./damage-v2.js";
import {
  COMBAT_ENCOUNTER_EXECUTION_POLICY_V2,
  COMBAT_ENCOUNTER_SCHEDULER_V2_VERSION,
  defineCombatEncounterStateV2,
  combatEncounterRoundOrderV2,
...
import {
  resolveCombatMovementV2,
} from "./movement-v2.js";
import {
  adjudicateCombatStatusActionV2,
  advanceCombatStatusBoundaryV2,
  mutateCombatStatusV2,
  resolveCombatForcedTargetV2,
  resolveCombatMovementAllowanceV2,
  combatStatusCombatModifiersV2,
...
import {
  collectCombatZoneTicksV2,
  endCombatZoneRoundV2,
  placeCombatZoneV2,
} from "./zones-v2.js";

export const COMBAT_ENCOUNTER_REDUCER_V2_VERSION = 1;
export const COMBAT_ENCOUNTER_COMMAND_TYPES_V2 = Object.freeze([
  "round-start",
  "actor-turn-start",
  "reaction-arm",
  "ability",
...
export const COMBAT_ENCOUNTER_COMBAT_RESULTS_V2 = Object.freeze([
  "victory",
  "defeat",
  "draw",
]);
export const COMBAT_ENCOUNTER_REDUCER_POLICY_V2 = COMBAT_ENCOUNTER_EXECUTION_POLICY_V2;

if (COMBAT_ENCOUNTER_REDUCER_POLICY_V2.version !== COMBAT_ABILITY_RULES_V2_VERSION
  || COMBAT_ENCOUNTER_REDUCER_POLICY_V2.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID
  || COMBAT_ENCOUNTER_REDUCER_POLICY_V2.reducerVersion
    !== COMBAT_ENCOUNTER_REDUCER_V2_VERSION) {
  throw new TypeError("combat-encounter-v2-reducer-policy-drift");
}
...
export const MAX_COMBAT_AI_STEP_RANDOM_DRAWS_V2 = 8_192;
export const COMBAT_ENCOUNTER_AI_STEP_STAGES_V2 = Object.freeze([
  "round-started",
  "actor-turn-started",
  "round-ended",
  "intent-declared",
...
function immutableState(value) {
  return defineCombatEncounterStateV2(value);
}
...
function replaceActorVitals(state, actorId, updates) {
  const actor = state.actors[actorId];
  return replaceActor(state, defineCombatActorV2({
    ...cloneData(actor),
    ...updates,
  }));
}
...
function expireDefeatedReaction(state, actorId) {
  ...
  const expired = expireCombatReactionV2(state.economy, {
    actorId,
...
function statusAdjustedScalar(state, actorId, basis, includeModifiers = true) {
  ...
  const modifiers = includeModifiers
    ? combatStatusCombatModifiersV2(state.statuses, actorId)
...
function statusSourceActorId(statusId, sourceActorId, operation) {
  ...
  const definition = getCombatStatusRulesV2(statusId);
...
function applyStatusMutation(state, {
  ...
  const mutation = mutateCombatStatusV2(state.statuses, {
    actorId,
    operation,
...
function applyShield(state, actorId, amount) {
  const actor = state.actors[actorId];
  if (!actor || actor.hp <= 0) return null;
  const shield = Math.min(COMBAT_ACTOR_SCALAR_MAX_V2, actor.shield + amount);
  return {
...
function damagePacket(state, {
  ...
  const resolved = resolveCombatDamageV2({
    source,
    target,
...
function applyZoneTick(state, tick, context) {
  ...
  if (tick.payload.primitive === "damage") {
    const damage = damagePacket(state, {
      ...
      amount: Math.min(COMBAT_DAMAGE_MAX_V2, amount),
      attackScaleBps: 0,
      kind: "periodic",
      randomDraws: [],
    });
  ...
  if (tick.payload.primitive === "resource") {
    ...
    const changed = applyCombatResolveDeltaV2(state.economy, {
      actorId: tick.actorId,
      delta,
    });
    return changed.ok
  ...
  for (const intent of intents) {
    if (intent.type === "damage") {
      ...
      const damage = damagePacket(current, {
        ...
        amount: Math.min(COMBAT_DAMAGE_MAX_V2, intent.amount),
        attackScaleBps: 0,
        kind: "periodic",
        randomDraws: [],
      });
    ...
    if (intent.type === "budget") {
      ...
      const changed = applyCombatActionBudgetDeltaV2(current.economy, {
        actorId: intent.actorId,
        lane: intent.lane,
        delta: intent.amount,
...
function advanceBoundary(state, boundary, actorId, options = {}) {
  const advanced = advanceCombatStatusBoundaryV2(state.statuses, { boundary, actorId });
...
function abilityAccessById(state, actorId, abilityId) {
  ...
  const definition = getCombatAbilityRulesV2(abilityId);
  ...
  const economy = canUseCombatAbilityV2(state.economy, {
    actorId,
    abilityId,
  });
...
function expectedDamageDraws(recipientSets, ability) {
  return ability.effects.reduce((total, effect, index) => (
    effect.primitive === "damage"
      ? total + (recipientSets[index].length * COMBAT_DAMAGE_POLICY_V2.direct.randomDrawsPerPacket)
      : total
  ), 0);
}
...
function matchingReactionSnapshot(state, hostileAbility, hostileCommit) {
  ...
  for (const [actorId, receipt] of Object.entries(state.reactionLocks)) {
    ...
    const reactionAbility = abilityRulesV2AtRank(
      getCombatAbilityRulesV2(arm.abilityId),
      arm.rank,
    );
...
function resolveReactionWindow(
  ...
  for (const entry of snapshot.entries) {
    ...
    const reactionAbility = abilityRulesV2AtRank(
      getCombatAbilityRulesV2(entry.abilityId),
      entry.rank,
    );
    ...
    const triggered = triggerCombatReactionV2(current.economy, {
      actorId: entry.actorId,
      abilityId: entry.abilityId,
...
function applyMovementEffect(state, effect, targetCommit, recipient, events) {
  const allowance = resolveCombatMovementAllowanceV2(state.statuses, {
  ...
  const resolved = resolveCombatMovementV2(state, effect, {
  ...
  for (let stepIndex = 0; stepIndex < resolved.movement.steps.length; stepIndex += 1) {
    ...
    if (step.enterTicks.length > 0 && stepIndex < resolved.movement.steps.length - 1) {
      const continued = resolveCombatMovementAllowanceV2(current.statuses, {
        actorId: recipient.actorId,
        requestedCells: effect.value.amount,
      });
...
function executeAbilityEffects(state, ability, targetCommit, recipientSets, randomDraws) {
  ...
  for (let effectIndex = 0; effectIndex < ability.effects.length; effectIndex += 1) {
    ...
    if (effect.primitive === "zone") {
      const definition = zoneRulesV2AtRank(COMBAT_ABILITY_ZONES_V2[effect.subject], ability.rank);
      let potency;
      try {
      ...
      for (let recipientIndex = 0; recipientIndex < recipients.length; recipientIndex += 1) {
        ...
        const placed = placeCombatZoneV2(current.zones, {
    ...
    for (let recipientIndex = 0; recipientIndex < recipients.length; recipientIndex += 1) {
      ...
      if (effect.primitive === "heal") {
        ...
        const changed = applyCombatResolveDeltaV2(current.economy, {
          actorId: recipient.actorId,
          delta,
        });
        if (!changed.ok) return { ok: false, reason: changed.reason };
...
function resolveClosedActorEndBoundaries(state, actorId) {
  ...
  if (occupant) {
    const collected = collectCombatZoneTicksV2(current.zones, {
      timing: "turn-end",
      occupants: [occupant],
    });
...
export function beginCombatEncounterRoundV2(state, input = {}) {
  ...
  const opened = beginCombatActionRoundV2(boundary.state.economy);
  if (!opened.ok) return failure(opened.reason, original);
  const order = combatEncounterRoundOrderV2(boundary.state);
  const next = {
    ...boundary.state,
    economy: opened.state,
    scheduler: {
      version: COMBAT_ENCOUNTER_SCHEDULER_V2_VERSION,
...
export function beginCombatEncounterActorTurnV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  ...
  const control = adjudicateCombatStatusActionV2(current.statuses, {
    actorId: input.actorId,
    lane: "main",
  });
  ...
  const opened = beginCombatActorTurnV2(current.economy, {
    actorId: input.actorId,
    budgetDeltas,
  });
  if (!opened.ok) return failure(opened.reason, original);
...
export function armCombatEncounterReactionV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  if (combatResult(original) !== null) return failure("encounter-v2-combat-complete", original);
  ...
  const definition = getCombatAbilityRulesV2(input.abilityId);
  ...
  const available = armCombatReactionV2(original.economy, {
    actorId: input.actorId,
  ...
  const events = [];
  const adjudicated = adjudicateCombatStatusActionV2(current.statuses, {
    actorId: input.actorId,
    lane: "reaction",
  });
  ...
  const armed = armCombatReactionV2(current.economy, {
    actorId: input.actorId,
    abilityId: input.abilityId,
    watchedActorId: locked.lock.anchor.actorId,
  });
...
function commitPreparedCombatEncounterAbilityV2(original, input, ability, plan) {
  const {
    recipientSets,
    reactionSnapshot,
    hostileExpectedDraws,
  ...
  const forced = resolveCombatForcedTargetV2(original.statuses, {
    actorId: input.actorId,
  ...
  const economy = commitCombatAbilityActionV2(current.economy, {
    actorId: input.actorId,
    abilityId: ability.id,
  });
  ...
  if (current.actors[input.actorId].hp <= 0
    ...
    const closed = endCombatActorTurnV2(current.economy, { actorId: input.actorId });
...
export function commitCombatEncounterAbilityV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  if (combatResult(original) !== null) return failure("encounter-v2-combat-complete", original);
  ...
  return commitPreparedCombatEncounterAbilityV2(original, input, ability, plan);
}

/** Close priority, then resolve pre-existing end-boundary status and zone effects. */
export function endCombatEncounterActorTurnV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  if (!exactKeys(input, ACTOR_INPUT_KEYS) || !actorIdentifier(input.actorId)) {
  ...
  if (combatResult(original) !== null) return failure("encounter-v2-combat-complete", original);
  const closed = endCombatActorTurnV2(original.economy, input);
  if (!closed.ok) return failure(closed.reason, original);
...
export function endCombatEncounterRoundV2(state, input = {}) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  if (!exactKeys(input, NO_INPUT_KEYS)) {
  ...
  events.push(...boundary.events);
  const zones = endCombatZoneRoundV2(current.zones, {
  ...
  const closed = endCombatActionRoundV2(current.economy);
  if (!closed.ok) return failure(closed.reason, original);
  current = { ...current, economy: closed.state };
...
function endAiPriorityAfterDecision(original, current, input, actorId, events) {
  const ended = endCombatEncounterActorTurnV2(current, { actorId });
  if (!ended.ok) return failure(ended.reason, original);
 ...
 */
export function runCombatEncounterAiStepV2(state, input) {
  const checked = checkedState(state);
  if (!checked.ok) return checked.result;
  const original = checked.state;
  if (!exactKeys(input, AI_STEP_INPUT_KEYS)
    || !randomDrawVector(input.randomDraws, MAX_COMBAT_AI_STEP_RANDOM_DRAWS_V2)) {
  ...
  if (original.economy.phase === "between-rounds") {
    const opened = beginCombatEncounterRoundV2(original);
    if (!opened.ok) return failure(opened.reason, original);
  ...
  if (original.economy.phase === "round") {
    const scheduled = scheduledProgress(original);
    if (scheduled.nextActorId === null) {
      const ended = endCombatEncounterRoundV2(original);
      if (!ended.ok) return failure(ended.reason, original);
      return aiStepResult(ended.state, ended.events, input, { stage: "round-ended" });
    }
    const actorId = scheduled.nextActorId;
    const opened = beginCombatEncounterActorTurnV2(original, { actorId });
    if (!opened.ok) return failure(opened.reason, original);
  ...
  if (intent === null) {
    ...
    const declared = declareCombatAiIntentV2(original, { actorId, declaredSequence });
  ...
  const evaluated = evaluateCombatAiIntentV2(original, { actorId, intent });
  ...
  if (!evaluated.valid) {
    ...
    const redeclared = redeclareCombatAiIntentV2(original, {
      actorId,
      intent,
  ...
  const executed = commitPreparedCombatEncounterAbilityV2(
    current,
    {
      actorId,
 ...
 */
export function reduceCombatEncounterV2(state, command) {
  const type = command?.type;
  const expected = COMBAT_ENCOUNTER_COMMAND_TYPES_V2.includes(type)
    ? COMMAND_KEYS[type]
    : null;
  if (!expected
    || !exactKeys(command, expected)
    || command.version !== COMBAT_ABILITY_RULES_V2_VERSION
    || command.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID) {
    const checked = checkedState(state);
    return checked.ok
      ? failure("invalid-encounter-v2-command", checked.state)
      : checked.result;
  }
  if (type === "round-start") return beginCombatEncounterRoundV2(state);
  if (type === "actor-turn-start") {
    return beginCombatEncounterActorTurnV2(state, { actorId: command.actorId });
  }
  if (type === "reaction-arm") {
    return armCombatEncounterReactionV2(state, {
  ...
  if (type === "ability") {
    return commitCombatEncounterAbilityV2(state, {
      actorId: command.actorId,
      abilityId: command.abilityId,
      anchor: command.anchor,
  ...
  if (type === "actor-turn-end") {
    return endCombatEncounterActorTurnV2(state, { actorId: command.actorId });
  }
  if (type === "ai-step") {
    return runCombatEncounterAiStepV2(state, { randomDraws: command.randomDraws });
  }
  return endCombatEncounterRoundV2(state);
}
