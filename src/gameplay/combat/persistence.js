...
import { COMBAT_COMMAND_TYPES } from "./commands.js";
import {
  MAX_COMBAT_COMMANDS,
  COMBAT_RULESET_ID,
...
function commandLogFailure(session) {
  ...
  for (let index = 0; index < commands.length; index += 1) {
    ...
    ids.add(command.id);
    if (!COMBAT_COMMAND_TYPES.includes(command.type)) return "invalid-command-type";
...
function receiptFailure(session) {
  ...
  if (!COMBAT_WORLD_FATES.includes(receipt.playerWorldFate)) return "invalid-terminal-receipt";
 ...
 */
export function decodeCombatSession(value) {
  let session;
  try {
    ...
    session = cloneJsonData(value, "invalid-combat-session-payload");
  } catch {
    return rejected("invalid-combat-session-payload");
  }

  if (session?.version !== COMBAT_SESSION_VERSION) return rejected("unsupported-combat-session-version");
  if (session?.rulesetId !== COMBAT_RULESET_ID) return rejected("unsupported-combat-ruleset");
  if (!isCombatSession(session)) return rejected("invalid-combat-session");

  if (session.encounter.round > MAX_COMBAT_ROUNDS) return rejected("combat-round-limit-exceeded");
  if (session.encounter.events.length > MAX_ENCOUNTER_EVENTS) return rejected("combat-event-limit-exceeded");
  if (session.commands.length > MAX_COMBAT_COMMANDS) return rejected("combat-command-limit-exceeded");
 ...
 */
export function encodeCombatSession(session) {
  const verified = decodeCombatSession(session);
...
export function isStoredCombatSession(value) {
