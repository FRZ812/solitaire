// Pure combat-query and presentation authority for solitaire-combat-v2.
//
...
import { verifyCombatSessionReplayV2 } from "./replay-v2.js";
import {
  MAX_COMBAT_SESSION_EVENTS_V2,
  combatEncounterCombatResultV2,
...
import {
  COMBAT_FORMATION_SIDES_V2,
  COMBAT_TARGET_COMMIT_V2_VERSION,
...
export const COMBAT_PRESENTATION_V2_VERSION = 1;

export const COMBAT_PRESENTATION_POLICY_V2 = deepFreeze({
  version: COMBAT_PRESENTATION_V2_VERSION,
...
function formationCell(value) {
  return exactKeys(value, TARGET_CELL_KEYS)
    && COMBAT_FORMATION_SIDES_V2.includes(value.side)
...
function canonicalSource(source) {
  if (isSessionCandidate(source)) {
    const session = validateCombatSessionV2(source);
    if (!session.ok) return { ok: false, reason: session.reason };
    const log = validateCombatCommandLogV2(source);
    if (!log.ok) return { ok: false, reason: log.reason };
    const replay = verifyCombatSessionReplayV2(source);
  ...
  const encounter = validateCombatEncounterStateV2(source);
  return encounter.ok
...
function decisionActorId(state) {
  if (combatEncounterCombatResultV2(state) !== null || state.economy.phase !== "actor-turn") {
    return null;
  }
  ...
  return Object.entries(state.intents)
    ...
    .map(([actorId, intent], index) => {
      ...
      return {
        ...
        name: getCombatAbilityNameV2(ability.id),
        rank: ability.rank,
...
export function projectArchetypeCombatViewV2(source) {
  ...
  const view = {
    ...
    rulesetId: COMBAT_ABILITY_RULESET_V2_ID,
    presentationVersion: COMBAT_PRESENTATION_V2_VERSION,
...
function resolvedAbility(state, actorId, abilityId) {
  ...
  const definition = equipped ? getCombatAbilityRulesV2(abilityId) : null;
...
function committedAnchors(state, actorId, ability) {
  ...
  const forced = resolveCombatForcedTargetV2(state.statuses, {
    actorId,
...
function legalAbilityContext(state, actorId, abilityId) {
  ...
  const economy = ability.action.lane === "reaction"
    ? armCombatReactionV2(state.economy, {
        actorId,
        abilityId,
        watchedActorId: anchors[0].lock.anchor.actorId,
      })
    : canUseCombatAbilityV2(state.economy, { actorId, abilityId });
  if (!economy.ok) return { ok: false, reason: economy.reason };
  const status = adjudicateCombatStatusActionV2(state.statuses, {
    actorId,
    lane: ability.action.lane,
  });
...
export function projectCombatLegalActionsV2(source) {
  const canonical = canonicalSource(source);
...
export function projectCombatLegalAnchorsV2(source, input) {
  const canonical = canonicalSource(source);
  if (!canonical.ok) return rejected("query", canonical.reason);
...
export function previewCombatAbilityTargetV2(source, input) {
  const canonical = canonicalSource(source);
...
export function projectCombatDeclaredIntentsV2(source, input = {}) {
  const canonical = canonicalSource(source);
...
function validTargetCommit(state, commit) {
  if (!exactKeys(commit, TARGET_COMMIT_KEYS)
    || commit.ok !== true
    || commit.reason !== null
    || commit.version !== COMBAT_TARGET_COMMIT_V2_VERSION
    || commit.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID
    ...
    || !COMBAT_FORMATION_SIDES_V2.includes(commit.anchor.side)
  ...
  const definition = getCombatAbilityRulesV2(commit.abilityId);
...
function validPresentationEvents(state, events) {
  if (!Array.isArray(events) || events.length > MAX_COMBAT_SESSION_EVENTS_V2) return false;
  try {
    ...
    for (let index = 0; index < snapshot.length; index += 1) {
      ...
      if (!event
        ...
        || (Object.hasOwn(event, "abilityId")
          && (!identifier(event.abilityId) || getCombatAbilityRulesV2(event.abilityId) === null))
        || !validEventTargetSnapshots(state, event)) return false;
      if (event.targetLock) {
        const definition = getCombatAbilityRulesV2(event.targetLock.abilityId);
...
export function projectCombatEventPresentationV2(source, input = {}) {
  const canonical = canonicalSource(source);
  if (!canonical.ok) return rejected("projection", canonical.reason);
  ...
  const payloads = snapshot.map((event) => {
    ...
    const rank = context?.targetCommit?.rank
      ...
      ?? null;
    const definition = abilityId === null ? null : getCombatAbilityRulesV2(abilityId);
    let ability = null;
    try {
