// Deterministic single-packet damage authority for solitaire-combat-v2.
//
...
import {
  ...
  isCombatStatusRuntimeV2,
  resolveCombatDirectHitStatusesV2,
  combatStatusCombatModifiersV2,
} from "./status-runtime-v2.js";

export const COMBAT_DAMAGE_KINDS_V2 = Object.freeze(["direct", "periodic"]);
export const COMBAT_DAMAGE_MAX_V2 = 1_000_000_000;
export const COMBAT_DAMAGE_ATTACK_SCALE_MAX_BPS_V2 = 100_000;
export const COMBAT_DAMAGE_ACTOR_SCALAR_MAX_V2 = COMBAT_ACTOR_SCALAR_MAX_V2;
...
export const COMBAT_DAMAGE_POLICY_V2 = deepFreeze({
  version: COMBAT_ABILITY_RULES_V2_VERSION,
  rulesetId: COMBAT_ABILITY_RULESET_V2_ID,
  packetKinds: COMBAT_DAMAGE_KINDS_V2,
  maximumPacketDamage: COMBAT_DAMAGE_MAX_V2,
  maximumAttackScaleBps: COMBAT_DAMAGE_ATTACK_SCALE_MAX_BPS_V2,
  maximumActorScalar: COMBAT_DAMAGE_ACTOR_SCALAR_MAX_V2,
  rates: {
    basisPoints: 10_000,
    minimum: 0,
    maximum: 10_000,
  ...
  statusPolicyChecksum: COMBAT_STATUS_POLICY_V2_CHECKSUM,
});
...
function validActor(value) {
  return isCombatActorV2(value);
}

export function validateCombatDamageActorV2(value) {
...
export function isCombatDamageActorV2(value) {
...
export function calculateCombatDamagePolicyV2Checksum() {
  return `fnv1a32:${fnv1a32(stableSerialize(COMBAT_DAMAGE_POLICY_V2))}`;
}

// Literal is checked by tests so formula or ordering drift cannot be accidental.
export const COMBAT_DAMAGE_POLICY_V2_CHECKSUM = "fnv1a32:f41dd5bb";

function clampRate(value) {
  return Math.max(
    COMBAT_DAMAGE_POLICY_V2.rates.minimum,
    Math.min(COMBAT_DAMAGE_POLICY_V2.rates.maximum, value),
  );
}
...
function validInput(input) {
  ...
  if (input.packet.kind === "direct") {
    if (!validActor(input.source)
      ...
      || input.randomDraws.length !== COMBAT_DAMAGE_POLICY_V2.direct.randomDrawsPerPacket
    ...
    || input.randomDraws.length !== COMBAT_DAMAGE_POLICY_V2.periodic.randomDrawsPerPacket) {
...
function directDamage(input) {
  const sourceModifiers = combatStatusCombatModifiersV2(input.statuses, input.source.id);
  const targetModifiers = combatStatusCombatModifiersV2(input.statuses, input.target.id);
  ...
  if (dodged) {
    const statusResult = resolveCombatDirectHitStatusesV2(input.statuses, {
  ...
  const afterCritical = critical
    ? Math.floor((afterAttackDelta * COMBAT_DAMAGE_POLICY_V2.direct.criticalMultiplierBps) / 10_000)
    : afterAttackDelta;
  ...
  const lifestealBps = Math.min(
    COMBAT_DAMAGE_POLICY_V2.lifesteal.capBps,
...
export function resolveCombatDamageV2(input) {
