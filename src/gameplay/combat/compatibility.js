...
// Solitaire combat kernel runs it, runs an adapted form of it, or refuses the encounter
...
/** How close a supported behaviour is to captured Solitaire combat evidence. */
export const FIDELITY = Object.freeze({
  /** Matches a captured value or rule in docs/design/COMBAT_EVIDENCE.md. */
  EXACT: "exact",
  /** A Solitaire combat mechanic reshaped to fit Solitaire's persistent world. */
  ADAPTED: "adapted",
  /** Solitaire-native, with no Solitaire combat counterpart. */
  EXTENDED: "extended",
...
export const DOMAIN_RULES = Object.freeze({
  "combat-trait": rule({
    ...
    source: "src/gameplay/combat/traits.js",
  }),
  "combat-fusion": rule({
    domain: "combat-fusion",
    support: SUPPORT.SUPPORTED,
  ...
  "combat-skill": rule({
    ...
    source: "src/gameplay/combat/skills.js",
  }),
  "combat-passive-skill": rule({
    domain: "combat-passive-skill",
    support: SUPPORT.SUPPORTED,
    fidelity: FIDELITY.EXACT,
  ...
  profession: rule({
    ...
    reason: "Solitaire professions have no Solitaire combat counterpart. Each maps to a "
      + "starting trait plus a skill loadout; the profession itself is not a combat entity.",
    source: "src/gameplay/combat/professions.js",
  }),
  "player-ability": rule({
    domain: "player-ability",
    support: SUPPORT.ABSENT,
    fidelity: FIDELITY.EXTENDED,
    reason: "Superseded by the profession's Solitaire combat package, which is the combat "
      + "identity by design rather than by omission. admitCombatEncounter records each "
    ...
    source: "src/gameplay/combat/admission.js",
  }),
  "racial-passive": rule({
    ...
    reason: "Racial passives are computed in progression code with no combat rule attached. "
      + "admitCombatEncounter records each one as superseded by the package rather than letting "
  ...
  companion: rule({
    ...
    support: SUPPORT.ADAPTED,
    fidelity: FIDELITY.EXTENDED,
    reason: "A combat-capable companion is fielded as an allied Solitaire combat actor under "
  ...
  condition: rule({
    ...
    support: SUPPORT.ADAPTED,
    fidelity: FIDELITY.EXTENDED,
    reason: "Each authored condition maps to an opening Solitaire combat status, or is "
...
export function capabilityInventory() {
  ...
  add(traitIds(), "combat-trait");
  add(fusionIds(), "combat-fusion");
  add(skillIds(), "combat-skill");
  add(passiveSkillIds(), "combat-passive-skill");
  add(statusTypes(), "combat-status");
