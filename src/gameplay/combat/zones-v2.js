// Pure spatial-zone authority for solitaire-combat-v2.
//
...
import { COMBAT_ABILITY_ZONES_V2 } from "./ability-catalog-v2.js";
import { FORMATION_CELLS } from "./formation.js";

export const COMBAT_ZONE_RUNTIME_V2_VERSION = 1;
export const MAX_COMBAT_ZONES_V2 = 4096;
export const MAX_COMBAT_ZONE_ROUNDS_V2 = 2000;
export const COMBAT_ZONE_TRIGGER_TIMINGS_V2 = Object.freeze([
  "after-enter",
  "turn-start",
  "turn-end",
  "round-end",
...
export const COMBAT_ZONE_RUNTIME_POLICY_V2 = deepFreeze({
...
function registryOrDefault(registry) {
  return registry === undefined ? COMBAT_ABILITY_ZONES_V2 : registry;
}
...
function instanceReason(value, registry) {
  ...
  if (!nonNegativeSafeInteger(value.resolvedPotency)
    ...
    || value.roundsRemaining > MAX_COMBAT_ZONE_ROUNDS_V2
...
export function validateCombatZoneInstanceV2(value, registry = COMBAT_ABILITY_ZONES_V2) {
  const reason = instanceReason(value, registryOrDefault(registry));
  return Object.freeze({ ok: reason === null, reason });
}

export function isCombatZoneInstanceV2(value, registry = COMBAT_ABILITY_ZONES_V2) {
  return validateCombatZoneInstanceV2(value, registry).ok;
}
...
function stateReason(value, registry) {
  ...
  if (!Array.isArray(value.zones) || value.zones.length > MAX_COMBAT_ZONES_V2) {
...
export function validateCombatZoneStateV2(value, registry = COMBAT_ABILITY_ZONES_V2) {
...
export function isCombatZoneStateV2(value, registry = COMBAT_ABILITY_ZONES_V2) {
  return validateCombatZoneStateV2(value, registry).ok;
}

export function createCombatZoneStateV2(input = {}) {
...
function placementReason(input, registry) {
  ...
  if (!actorIdentifier(input.instanceId)
    ...
    || input.rounds < 1
    || input.rounds > MAX_COMBAT_ZONE_ROUNDS_V2
 ...
 */
export function placeCombatZoneV2(
  state,
  input,
  options = {},
) {
  ...
  if (!existing && current.zones.length >= MAX_COMBAT_ZONES_V2) {
  ...
  const validation = validateCombatZoneStateV2(next, selectedRegistry);
...
export function collectCombatZoneTicksV2(
  state,
  input,
  options = {},
) {
  ...
  if (!exactKeys(input, TRIGGER_KEYS)
    || !COMBAT_ZONE_TRIGGER_TIMINGS_V2.includes(input.timing)
...
export function endCombatZoneRoundV2(
  state,
  input,
  options = {},
) {
  ...
  const collected = collectCombatZoneTicksV2(current, {
    timing: "round-end",
