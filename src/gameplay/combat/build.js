...
// (combatBuildForCharacter), which means it cannot grow: a trait earned from a trainer, a
...
export const COMBAT_BUILD_VERSION = 2;
const MAX_RUNES = 32;
...
export function isLegacyCombatBuild(value) {
...
export function migrateLegacyCombatBuild(value) {
  if (!isLegacyCombatBuild(value)) return null;
  try {
    return createCombatBuild({
...
export function createCombatBuild({ professionId, traits = {}, skills = [], runes = [] } = {}) {
  ...
  return canonicalizeCombatBuild({
    version: COMBAT_BUILD_VERSION,
    professionId,
    traits: cleanTraits,
    skills: cleanSkills,
    runes: [...new Set(runes)],
 ...
 */
export function canonicalizeCombatBuild(build) {
  const traits = {};
...
export function isCombatBuild(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const keys = Object.keys(value).sort();
  if (keys.length !== BUILD_KEYS.length || keys.some((key, at) => key !== BUILD_KEYS[at])) {
    return false;
  }
  if (value.version !== COMBAT_BUILD_VERSION || !validId(value.professionId)) return false;
...
export function acquireTrait(build, traitId) {
  ...
  return { ok: true, reason: null, build: canonicalizeCombatBuild({ ...build, traits }) };
}
...
export function acquireRune(build, runeId) {
  ...
  return {
    ok: true,
    reason: null,
    build: canonicalizeCombatBuild({ ...build, runes: [...build.runes, runeId] }),
  };
}
