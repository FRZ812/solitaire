...
import { createCombatEncounter, isCombatEncounter } from "./encounter.js";
import {
  STATIC_FORMATION_RULES_VERSION,
  isFormationRulesVersion,
  normalizeFormation,
} from "./formation.js";
import { COMBAT_RULESET_ID, COMBAT_SESSION_VERSION } from "./ruleset.js";

export { COMBAT_RULESET_ID, COMBAT_SESSION_VERSION } from "./ruleset.js";

/** The generator behind every stream. Bumping this invalidates replay on purpose. */
export const COMBAT_RNG_VERSION = "mulberry32-v1";
 ...
 */
export const COMBAT_RNG_STREAMS = Object.freeze(["combat", "intent", "loot", "rewards"]);

/** The streams the session itself carries; the rest belong to the encounter. */
export const COMBAT_SESSION_STREAMS = Object.freeze(["loot", "rewards"]);

export const COMBAT_SESSION_MODES = Object.freeze(["campaign", "practice"]);
export const COMBAT_SESSION_STATUSES = Object.freeze(["active", "terminal", "settled"]);
export const COMBAT_LETHAL_POLICIES = Object.freeze(["nonlethal", "lethal", "mixed"]);
 ...
 */
export const COMBAT_PLAYER_STAKES = Object.freeze(["survivable", "lethal"]);

export const COMBAT_RETREAT_POLICIES = Object.freeze(["forbidden", "allowed"]);
...
export function deriveSeedManifest(rootSeed) {
  ...
  return Object.fromEntries(
    COMBAT_RNG_STREAMS.map((name) => [name, `${rootSeed}::combat-stream::${name}::v1`]),
  );
}

function isSeedManifest(value) {
  return exactKeys(value, [...COMBAT_RNG_STREAMS].sort())
    && COMBAT_RNG_STREAMS.every((name) => identifier(value[name]));
}
...
function isParticipantBindings(value) {
  ...
  if (entries.length > MAX_COMBAT_PARTICIPANTS) return false;
...
function isContext(value) {
  ...
  return optionalIdentifier(value.directiveId)
    ...
    && COMBAT_LETHAL_POLICIES.includes(value.lethalPolicy)
    && COMBAT_PLAYER_STAKES.includes(value.playerStakes)
    && COMBAT_RETREAT_POLICIES.includes(value.retreatPolicy)
...
export function archetypeCombatContext(input = {}) {
  ...
  return {
    ...
    lethalPolicy: COMBAT_LETHAL_POLICIES.includes(input.lethalPolicy) ? input.lethalPolicy : "nonlethal",
    playerStakes: COMBAT_PLAYER_STAKES.includes(input.playerStakes) ? input.playerStakes : "survivable",
    retreatPolicy: COMBAT_RETREAT_POLICIES.includes(input.retreatPolicy) ? input.retreatPolicy : "allowed",
    lootPolicy: {
...
function isGenesis(value) {
  ...
  return isSeedManifest(value.seedManifest)
    ...
    && value.enemySnapshots.length < MAX_COMBAT_PARTICIPANTS
    && Array.isArray(value.allySnapshots)
    && value.allySnapshots.length < MAX_COMBAT_PARTICIPANTS
...
export function combatSessionChecksum(session) {
...
export function sealCombatSession(session) {
  const sealed = { ...session, checksum: null };
  sealed.checksum = combatSessionChecksum(sealed);
  return sealed;
}
 ...
 */
export function combatStreamEndpoints(session) {
  return {
...
export function markCombatSessionSettled(session, settlementId) {
  ...
  const settled = sealCombatSession({
    ...session,
    status: "settled",
    settlementId,
    checksum: null,
 ...
 * replay divergence.
 */
export function spendCombatSessionStream(session, name, endpoint) {
...
export function isCombatSession(value) {
  if (!exactKeys(value, SESSION_KEYS)) return false;
  if (value.version !== COMBAT_SESSION_VERSION) return false;
  if (value.rulesetId !== COMBAT_RULESET_ID) return false;
  if (!identifier(value.sessionId)) return false;
  if (!COMBAT_SESSION_MODES.includes(value.mode)) return false;
  if (!COMBAT_SESSION_STATUSES.includes(value.status)) return false;
  ...
  if (!isCombatEncounter(value.encounter)) return false;
  ...
  if (!exactKeys(value.streams, [...COMBAT_SESSION_STREAMS].sort())) return false;
  if (!COMBAT_SESSION_STREAMS.every((name) => isRngState(value.streams[name]))) return false;
...
export function createCombatSession(input = {}) {
  ...
  if (!COMBAT_SESSION_MODES.includes(mode)) return rejected("invalid-session-mode");
  ...
  const session = sealCombatSession({
    version: COMBAT_SESSION_VERSION,
    sessionId,
  ...
  if (!isCombatSession(session)) return rejected("invalid-session");
