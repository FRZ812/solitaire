// Solitaire professions as Solitaire combat starting packages.
//
...
// from the Solitaire combat catalogue, and nothing else about it reaches combat.
//
...
// are slots in Solitaire combat rather than cards — a replacement takes the slot over.
...
export function combatPackageForProfession(professionId) {
 ...
 * Solitaire combat number. The curve is a judgement, not evidence: rank 1 at level 1 up to
...
export function combatBuildForCharacter(character = {}) {
  const definition = combatPackageForProfession(character.profession);
