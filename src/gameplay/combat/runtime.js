...
export const COMBAT_RUNTIME_REASONS = Object.freeze({
  invalidIdentity: "invalid-combat-runtime-identity",
  legacyRuntime: "unsupported-legacy-combat-runtime",
  unsupportedRuntime: "unsupported-combat-runtime",
});

/** The exact identity of the sole registered runtime. */
export const COMBAT_V1_RUNTIME_IDENTITY = Object.freeze({
...
function isRetiredIdentity(identity) {
  return COMBAT_RETIRED_RUNTIME_IDENTITIES.some((retired) => (
 ...
 * definition. Adding `{version: 2, rulesetId: "solitaire-combat-v2"}` later is a separate
 * registration and cannot be inferred from any v1 payload detail.
 */
export function combatRuntimeRegistrationKey(identity) {
...
export function combatRuntimeIdentity(value) {
  const fields = identityFields(value);
  return fields ? Object.freeze(fields) : null;
}

const V1_RUNTIME = Object.freeze({
  identity: COMBAT_V1_RUNTIME_IDENTITY,
  create: createCombatSession,
  decode: decodeCombatSession,
  encode: encodeCombatSession,
  ...
  sequenceStream(session, name) {
    return COMBAT_SESSION_STREAMS.includes(name)
      ? streamSequencer(session.streams[name])
      : null;
  },
  settleEncounter(state, session, context) {
    return settleCombatEncounter(state, session.encounter, context);
  },
  spendStream: spendCombatSessionStream,
  markSettled: markCombatSessionSettled,
});
...
const RUNTIMES = new Map([
  [combatRuntimeRegistrationKey(COMBAT_V1_RUNTIME_IDENTITY), V1_RUNTIME],
]);

/** Read-only registration metadata for diagnostics and future migration UI. */
export const COMBAT_RUNTIME_IDENTITIES = Object.freeze([COMBAT_V1_RUNTIME_IDENTITY]);
...
function route(value) {
  ...
  if (!identity) {
    return {
      ok: false,
      reason: COMBAT_RUNTIME_REASONS.invalidIdentity,
      identity: null,
      runtime: null,
    };
  }
  const runtime = RUNTIMES.get(combatRuntimeRegistrationKey(identity)) || null;
  if (!runtime && isRetiredIdentity(identity)) {
    return {
      ok: false,
      reason: COMBAT_RUNTIME_REASONS.legacyRuntime,
  ...
  return runtime
    ...
    : {
        ok: false,
        reason: COMBAT_RUNTIME_REASONS.unsupportedRuntime,
        identity: Object.freeze(identity),
        runtime: null,
      };
}
 ...
 */
export function createCombatRuntimeSession(identity, input = {}) {
...
export function replayCombatRuntimeSession(session) {
...
export function combatRuntimeSessionEvents(session) {
  const routed = route(session);
...
export function combatRuntimeWorldFates(session) {
  const routed = route(session);
