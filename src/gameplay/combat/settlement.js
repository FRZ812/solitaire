// Folding a finished Solitaire combat encounter back into campaign state.
//
// Generalised from the single-enemy production settlement: a Solitaire combat encounter
...
function rejected(reason, state, receipt = null) {
  return { ok: false, reason, state, receipt, duplicate: reason === "combat-encounter-already-settled" };
}
 ...
 * @param {object} encounter a terminal COMBAT encounter
 ...
 *   not for another.
 */
export function settleCombatEncounter(state, encounter, context = {}) {
  ...
  try {
    ...
    return rejected("invalid-combat-settlement-receipts", state);
  }
  ...
  try {
    ...
    return rejected("invalid-combat-settlement-campaign-state", state);
  }
  const usesLegacyProgression = usesLegacyCharacterProgression(character);
  if (!usesLegacyProgression) stripCombatLegacyProgression(character);
  ...
  if (Number.isFinite(player.resolve)) {
    const appliedMaxBonus = Math.max(0, Math.round(combatItemActorBonuses(
      activeCombatItemIds(character, state.world?.codex),
    ).resolveMax || 0));
  ...
  if (wanderer) {
    const projection = {
      ...
      ...(character.combatBaseStats ? { combatBaseStats: cloneJsonData(character.combatBaseStats) } : {}),
      ...(usesLegacyProgression && character.progression
        ? { progression: cloneJsonData(character.progression) }
        : {}),
    };
    if (!usesLegacyProgression) stripCombatLegacyProgression(projection, { forceCombat: true });
  ...
  return {
    ...
    state: {
      ...
      beats: [
        ...(state.beats || []),
        { id: `archetype-combat:${encounterId}:settled`, type: "narration", content },
        ...(growthText
          ? [{ id: `archetype-combat:${encounterId}:growth`, type: "growth", text: growthText }]
          : []),
      ],
    },
  };
