// Pure stepwise movement authority for solitaire-combat-v2.
//
...
export const COMBAT_MOVEMENT_V2_VERSION = 1;
export const COMBAT_MOVEMENT_SIDES_V2 = Object.freeze(["player", "enemy"]);
export const COMBAT_MOVEMENT_STOP_REASONS_V2 = Object.freeze([
  "boundary",
  "occupied",
  "zone-block-exit",
  "zone-block-entry",
]);
export const COMBAT_MOVEMENT_POLICY_V2 = deepFreeze({
...
function side(value) {
  return COMBAT_MOVEMENT_SIDES_V2.includes(value);
}
...
function stateReason(state) {
  ...
  const seen = new Set();
  try {
    for (const formationSide of COMBAT_MOVEMENT_SIDES_V2) {
...
export function validateCombatMovementStateV2(state) {
...
export function validateCombatMovementEffectV2(effect) {
...
export function validateCombatMovementContextV2(context, effect) {
  const effectValidation = validateCombatMovementEffectV2(effect);
...
function movementSummary(context, effect, fromCell, currentCell, requestedDistance,
  ...
  return {
    version: COMBAT_MOVEMENT_V2_VERSION,
 ...
 */
export function resolveCombatMovementV2(
  state,
  effect,
  context,
  options = {},
) {
  const stateValidation = validateCombatMovementStateV2(state);
  ...
  const contextValidation = validateCombatMovementContextV2(context, effect);
  ...
  if (zones !== null) {
    const zoneValidation = validateCombatZoneStateV2(zones, registry);
  ...
  for (let ordinal = 1; ordinal <= plannedDistance; ordinal += 1) {
    ...
    let enterTicks = [];
    if (zones !== null) {
      const entered = collectCombatZoneTicksV2(zones, {
        timing: "after-enter",
        occupants: [{
          actorId: context.moverId,
          actorSide: mover.side,
