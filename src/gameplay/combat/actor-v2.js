// Strict actor snapshots for solitaire-combat-v2.
//
...
export const COMBAT_ACTOR_SIDES_V2 = Object.freeze(["player", "enemy"]);
export const COMBAT_ACTOR_CONTROLLERS_V2 = Object.freeze(["human", "ai"]);
export const COMBAT_ACTOR_PREFERRED_ROWS_V2 = Object.freeze([0, 1, 2]);
export const COMBAT_ACTOR_SCALAR_MAX_V2 = 1_000_000_000;
...
function actorReason(value) {
  ...
  if (value.version !== COMBAT_ABILITY_RULES_V2_VERSION
    || value.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID) return "invalid-actor-v2-ruleset";
  if (!actorIdentifier(value.id) || !canonicalName(value.name)) {
    return "invalid-actor-v2-identity";
  }
  if (!COMBAT_ACTOR_SIDES_V2.includes(value.side)
    || !COMBAT_ACTOR_CONTROLLERS_V2.includes(value.controller)
    || !COMBAT_ACTOR_PREFERRED_ROWS_V2.includes(value.preferredRow)) {
...
export function validateCombatActorV2(value) {
  const reason = actorReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isCombatActorV2(value) {
  return validateCombatActorV2(value).ok;
}

/** Validate, detach, and deeply freeze one complete actor snapshot. */
export function defineCombatActorV2(value) {
  const validation = validateCombatActorV2(value);
...
export function createCombatActorV2(input) {
  ...
  const validation = validateCombatActorV2(candidate);
