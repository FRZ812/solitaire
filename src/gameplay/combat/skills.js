// The Solitaire combat skill catalogue and its cooldown/resource state machine, transcribed
// from docs/design/COMBAT_EVIDENCE.md.
//
...
import { canonicalCombatArchetypeId } from "./archetype-identities.js";
import { withFunctionalPromotions } from "./ability-progression.js";
import {
  COMBAT_GENERAL_ABILITY_SOURCE_ROWS,
  COMBAT_GENERAL_SOURCE_CAPTURE,
...
export const GENERAL_ACTIVE_SOURCE = COMBAT_GENERAL_SOURCE_CAPTURE.url;
export const GENERAL_ABILITY_IDS = Object.freeze(
  COMBAT_GENERAL_ABILITY_SOURCE_ROWS.map((row) => row.id),
);
...
function skill(id, name, {
  ...
  return Object.freeze({
    ...
    exclusiveTo,
    archetypeId: canonicalCombatArchetypeId(exclusiveTo),
    abilityType,
    description,
...
function compileGeneralAbility(row) {
  ...
  return skill(row.id, row.name, {
    ...
    description,
    source: {
      page: COMBAT_GENERAL_SOURCE_CAPTURE.url,
      artifact: COMBAT_GENERAL_SOURCE_CAPTURE.artifact,
      sourceName: row.name,
      sourceOriginalName: row.sourceName,
      sourceLine: row.sourceLine,
      rawRowsSha256: COMBAT_GENERAL_SOURCE_CAPTURE.rawRowsSha256,
      fidelity: "adapted",
...
const GENERAL_SKILLS = Object.freeze(COMBAT_GENERAL_ABILITY_SOURCE_ROWS.map(compileGeneralAbility));
