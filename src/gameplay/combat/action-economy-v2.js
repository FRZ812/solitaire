// Pure action-economy authority for the opt-in solitaire-combat-v2 ruleset.
//
...
export const COMBAT_ACTION_ECONOMY_PHASES_V2 = Object.freeze([
  "between-rounds",
  "round",
  "actor-turn",
]);

export const COMBAT_ACTION_BUDGET_BASE_V2 = Object.freeze({
  main: 1,
  quick: 1,
  reaction: 1,
});
...
export const COMBAT_ACTION_BUDGET_CAP_V2 = Object.freeze({
  main: 3,
  quick: 3,
  reaction: 1,
});

// The scheduler must adjudicate control/status effects before it calls `beginCombatActorTurnV2`.
...
export const COMBAT_ACTION_ECONOMY_POLICY_V2 = deepFreeze({
  ownerPriorityOrder: [
  ...
  baseBudgets: COMBAT_ACTION_BUDGET_BASE_V2,
  budgetCaps: COMBAT_ACTION_BUDGET_CAP_V2,
...
function stateReason(value) {
  ...
  if (value.rulesetId !== COMBAT_ABILITY_RULESET_V2_ID) {
  ...
  if (!COMBAT_ACTION_ECONOMY_PHASES_V2.includes(value.phase)) {
  ...
  for (const actorId of actorIds) {
    ...
    if (!exactKeys(actor.budgets, BUDGET_KEYS)
      || BUDGET_KEYS.some((lane) => !Number.isSafeInteger(actor.budgets[lane])
        ...
        || actor.budgets[lane] > COMBAT_ACTION_BUDGET_CAP_V2[lane])) {
    ...
    for (const [abilityId, count] of Object.entries(actor.cooldowns)) {
      ...
      const action = abilityRulesV2AtRank(
        getCombatAbilityRulesV2(abilityId),
...
function canonicalStateOrFailure(state) {
  const validation = validateCombatActionEconomyV2(state);
...
function actionAccess(state, input, expectedLane = null) {
  ...
  const action = abilityRulesV2AtRank(
    getCombatAbilityRulesV2(input.abilityId),
...
export function validateCombatActionEconomyV2(value) {
  const reason = stateReason(value);
  return Object.freeze({ ok: reason === null, reason });
}

export function isCombatActionEconomyV2(value) {
  return validateCombatActionEconomyV2(value).ok;
}

export function createCombatActionEconomyV2(input) {
  ...
  for (const value of input.actors) {
    ...
    for (const ability of value.abilities) {
      ...
      const definition = getCombatAbilityRulesV2(ability.id);
...
export function beginCombatActionRoundV2(state) {
...
export function endCombatActionRoundV2(state) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function beginCombatActorTurnV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
  ...
  const budgets = Object.fromEntries(BUDGET_KEYS.map((lane) => [
    lane,
    clampedIntegerDelta(
      COMBAT_ACTION_BUDGET_BASE_V2[lane],
      budgetDeltas[lane],
      COMBAT_ACTION_BUDGET_CAP_V2[lane],
    ),
  ]));
  const nextActor = {
    ...cloneActor(actor),
...
export function endCombatActorTurnV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function canUseCombatAbilityV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
...
export function commitCombatAbilityActionV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function armCombatReactionV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function triggerCombatReactionV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function expireCombatReactionV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function applyCombatResolveDeltaV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
...
export function applyCombatActionBudgetDeltaV2(state, input) {
  const checked = canonicalStateOrFailure(state);
  if (!checked.ok) return checked.result;
  const current = checked.state;
  ...
  const after = clampedIntegerDelta(before, input.delta, COMBAT_ACTION_BUDGET_CAP_V2[input.lane]);
  return success(
