...
// carried into a Solitaire combat fight. The live path never asked it. So a character walked
...
//                 The profession's Solitaire combat package replaces the ability list; that
...
export const COMBAT_ADMISSION_VERSION = 1;
 ...
 *
 * Most of these are near-direct: Solitaire named its conditions and Solitaire combat named
 ...
 *
 * Counts are opening magnitudes, not durations, because Solitaire combat statuses are counts.
...
export const CONDITION_COMBAT_EFFECTS = Object.freeze({
  ...
  "Silenced": noCombatEffect("The Solitaire combat package has no spoken component to cut off; "
 ...
 */
export function admitCombatEncounter({ character = {}, party = [], enemies = [] } = {}) {
  const blockers = [];
  const notes = [];
  ...
  // The profession's Solitaire combat package *is* the combat identity — that is the whole
  ...
  for (const ability of character.abilities || []) {
    ...
    notes.push(note(ADMISSION_DISPOSITION.SUPERSEDED, "ability-superseded-by-package", {
      abilityId,
      reason: "The profession's Solitaire combat trait and skill loadout replaces the legacy "
  ...
  return {
    version: COMBAT_ADMISSION_VERSION,
