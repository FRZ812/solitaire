...
export const COMBAT_INTENT_VERSION = 1;
 ...
 *
 * There is no Solitaire combat evidence either way — telegraphs are a Solitaire adaptation —
...
export function isCombatIntent(value) {
  return exactKeys(value, INTENT_KEYS)
    && value.version === COMBAT_INTENT_VERSION
 ...
 */
export function declareCombatIntent({ schedule, declarationIndex, targetId, targets, rng }) {
...
export function advanceCombatIntent({ schedule, intent, targetId, targets, rng }) {
  return declareCombatIntent({
    schedule,
...
export function resolveDeclaredAttack(intent, attacks) {
  if (!isCombatIntent(intent) || !Array.isArray(attacks)) return null;
 ...
 */
export function describeCombatIntent(intent, attacks) {
