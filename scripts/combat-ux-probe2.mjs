...
async function tapStrikeAtEnemy() {
  ...
  await page.waitForFunction(() => (
    document.querySelector('[data-testid="combat-target-confirmation"]')
    || document.querySelector(".archetype-combat")?.getAttribute("aria-busy") === "true"
  ), { timeout: 5000 });
  const hasConfirm = await page.evaluate(() => !!document.querySelector('[data-testid="combat-target-confirmation"]'));
  if (hasConfirm) {
    await page.evaluate(() => {
      const cell = document.querySelector('[aria-label="Enemy formation"] .combat-formation-cell.has-unit:not(:disabled)');
...
for (let turn = 0; turn < 14; turn += 1) {
  const over = await page.evaluate(() => ({
    outcome: !!document.querySelector(".archetype-combat__outcome"),
    combat: !!document.querySelector(".archetype-combat"),
  }));
  if (over.outcome || !over.combat) break;

  // who is the active commander? read the glowing unit + status line
  const active = await page.evaluate(() => ({
    status: document.querySelector(".archetype-combat__round em")?.textContent,
    activeUnit: document.querySelector(".combat-formation-unit.is-active")?.getAttribute("aria-label") || null,
  }));
...
const outcome = await page.evaluate(() => ({
  heading: document.querySelector(".archetype-combat__outcome-heading")?.textContent,
  tally: document.querySelector(".archetype-combat__tally")?.textContent,
  buttons: [...document.querySelectorAll(".archetype-combat__settle")].map((b) => b.textContent),
}));
...
await page.evaluate(() => {
  const settle = document.querySelector(".archetype-combat__settle");
