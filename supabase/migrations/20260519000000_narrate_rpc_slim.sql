...
-- Drop the prompt column if the older migration created it.
alter table public.narrator_config drop column if exists system_prompt;
insert into public.narrator_config (id) values (1)
...
begin
  ...
  v_body := jsonb_build_object(
    'systemInstruction', jsonb_build_object(
      'parts', jsonb_build_array(jsonb_build_object('text', system_prompt))),
    'contents', v_contents,
