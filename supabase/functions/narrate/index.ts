...
- WHITEMARCH is enclosed by the GREAT WALL; the ONLY way through it is the CROWN GATE. The player starts INSIDE, at the central Grand Market plaza. The city is laid out as a SPACIOUS street grid with BUILDING islands set into it — every building has streets on three or more sides, and every cluster (the Grand Market, the Chain Ward, the Citadel) is ringed by lanes. When describing travel, walk the player ALONG those lanes past the district fronts; never narrate them as cutting through a stall or a hall to reach the next street. REFER TO PLACES BY NAME, NOT COORDINATE — the [STATE] line and the [LOCAL PLACE] / [SERVICE] tags built each turn already give you the player's exact hex, the POI footprint they stand on, the part of the building they're in, and what's open right now. The map is the source of truth; let it handle navigation, and let the engine spawn the trade button on the right tile. The wards, by district:
  - The Crown Gate complex — the only passage through the Great Wall. The gatehouse-towers straddle the wall (toll is paid under the murder-holes); the Crown Road Approach is the staging road OUTSIDE the wall where the queue forms before the gate; the inner gate-square opens into the city's main streets, with the old Customs Hall, the Chandler's Stall, and the porter-lane just inside.
  - Wall command — the Dragon-Watch Tower on the northeast wall over the Whitewend (anti-dragon station, restricted), and the Crown Guardpost (gate-guard barracks set into the wall beside the Toll Hall).
  - The Grand Market — a footprint of merged tiles around a central plaza: Grain Square is the plaza (the START); the stall-houses ringing it are Butchers' Row (covered meat-hooks), Cloth Awnings (covered cloth-stalls), Night Market (shuttered stall-house used unofficially after dark), and the Coin Scales (weighing-and-stamp bench); the Fishmonger's Bench sits on the lane between the market and the High Quay.
  - Caravan Yard & Stable — caravans, hire-boards, mounts, in the city's NW corner. River Docks line the eastern wall on the Whitewend (the whole eastern face of the city looks onto the river).
  - Halfborn Hostel — the Halfborn Hold's town-lodge for freedfolk on city business. Embassy Lane — foreign compounds.
  - The Chain Ward — Chain Market Steps (the public sale-platform plus the petition-rail and viewing-yard pressed against it), and Registry Hall (status/leases/recovery-writs at the lease-desk and the public-counters). The trade button for the Block appears on the sale-platform tile only.
  - Iron Quarter on the river — Public Smith Row: riverside forges and state foundries, smoke leaning out over the Whitewend.
  - Courts row — Lower Petition Steps (the open court approach) and the Guild Court (the polished-slate hall at the eastern river corner — licenses and craft disputes).
